...
Your agent calls discern's tools over the **Model Context Protocol (MCP)**, the connection coding agents use to call tools, and when that connection has a problem, your agent can use the `discern` command line instead. Say your agent is writing help pages for your recipe app, waiting for the recipe search task to pass its checks, when its discern tools disappear. Ask your agent:
...
Your agent can keep working while you fix this: it runs `discern status --json` from the task's worktree, its separate copy of the project, to see where things stand, then matches what it sees:
...
Your agent waits for another task with `discern_await`, and each call waits as long as your coding agent reliably allows, up to 55 minutes. If recipe search hasn't passed its checks by then, the call still returns `ok: true`, with `data.met: false` and a handle to continue it. The wait isn't over, so the agent calls again with the newest handle:
...
On the command line, that's `discern await --resume C1-BKJD-X4GQ-05`, which exits with code `124` while the wait isn't met. Use the handle from your own result. The handle keeps the original condition and still notices a change that happened between calls, so the agent needs no delay or loop.

The wait is over when `data.met` is `true`, or when you decide the help pages no longer need search.

A refusal is different: it has `ok: false`, resuming won't help, and the result says why:
...
Sometimes your coding agent stops waiting before discern replies, which doesn't tell you whether the command finished. So before running it again only to see the output, your agent reads the run back:

```sh
discern progress
```

With no handle, it reads the most recent run started in that worktree. With the handle the run announced when it started, such as `R1-H596-N6BT-K5`, it reads that run. Then `discern status` shows where the task stands.
...
If you can't tell which of these happened, read the run back. discern has no separate cancel command, so you stop a run from wherever it started.

To see where a run got to, your agent runs `discern progress` with its handle. The reading says the run is still going, finished, `was cancelled before finishing`, or `stopped without finishing`, which means its process died.
...
When the agent runs `discern done` again after a stopped run, commands that didn't finish run again, and those that finished and declare their inputs reuse their results when those inputs haven't changed. A landing that stopped partway has its own recovery: [Recover an interrupted acceptance](../20-guides/recover-an-interrupted-task.md#recover-an-interrupted-acceptance).
