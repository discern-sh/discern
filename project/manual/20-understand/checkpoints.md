...
The built-in checkpoints that watch code changes only advise, so none of them blocks a code change. The ones that watch changes to your project's instructions, skills, and map stop for an answer, because every later session reads those. Your project chooses which checkpoints to use and can change any of them. `discern checkpoints` lists the ones that apply to yours.
