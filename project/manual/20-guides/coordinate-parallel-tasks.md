...
The **desk** is the interactive view that opens when you run `discern` in your main checkout. It lists every task and shows which ones need a decision from you. It also lists each task's actions, and marks the ones that can't run right now as unavailable. Pick one of those and the desk tells you why. It keeps itself up to date, so you can leave it open and come back when a task needs you. You don't have to open each agent's session to ask how it's going.
