...
| `--theme <theme>` | Set the terminal theme to `auto`, `light`, or `dark`. Default: `auto`. The automatic mode senses a coloured interactive background; `--no-color` and `NO_COLOR` skip sensing. |
