...
discern runs on macOS and Linux. On Windows, use WSL 2. The binary includes its own runtime, but your project still needs the tools its own checks use.
...
| Running discern                | `git` and a POSIX `sh` on `PATH`. discern runs the commands in your gate, the checks a change must pass, and your resource commands through `sh -c`.                                                                                             |
...
Doctor checks the configuration, the required tools, the integrations, and the project setup. It doesn't repair files or run generators. Its result explains each problem and the next action. [Setup and integrations troubleshooting](../40-troubleshooting/setup-and-integrations.md) helps when a check fails.
...
Once installed, discern makes no network calls itself. Your project's own commands can still use the network.
...
Every command must succeed, and the `codesign -dv` output must name a Developer ID Application authority. The attestation commands need a public release, and a GitHub CLI login that can verify attestations.

### Values that differ by worktree

Each task works in its own worktree, and discern derives values from its identity that you can use in your tooling:
...
This table compares each tool's launcher, files, trust step, and limits. MCP, the Model Context Protocol, is how a coding tool calls discern's tools directly.
...
The agent file is the file each tool reads its instructions from. When Codex, Cursor, or GitHub Copilot is configured, discern writes the full instructions into `AGENTS.md`, and `CLAUDE.md` and `GEMINI.md` hold only the pointer `@AGENTS.md`. When only Claude Code, Gemini, or both are configured, `CLAUDE.md` and `GEMINI.md` hold the full instructions, and discern writes no `AGENTS.md`.

The last column gives each tool's limit for one MCP call, then the longest `discern_await` request discern makes there. Most tools let an MCP call run for 3,600 seconds. `discern_await` uses at most 3,300 seconds of that, which leaves 5 minutes for the tool to deliver the result. Cursor has the shorter limits shown.

Session hooks have their own limit of 600 seconds. Gemini records it as 600,000 milliseconds, and the other tools use seconds. `discern refresh` brings the settings discern owns up to each tool's current format and keeps unrelated settings unchanged. The file locations and ownership rules stay the same when a format changes.

When you run setup without naming tools, it selects every tool it finds installed: the tool's launcher on `PATH`, or for Cursor also the editor's `cursor` command or its usual application location. If it finds none, it selects Claude Code and Codex. discern also uses Claude Code and Codex when `[project].agents` isn't set.
...
A coding tool loads discern's MCP server, hooks, and rules when a session starts, so the session that ran setup can't see them. After setup or an upgrade, start a fresh session. In it, your agent checks the tools the session registered, then calls the activation check below. Activation is confirmed only when that call returns. Files on disk don't prove that the session loaded the integration.
...
Git keeps the agent instruction files and the provider configuration. It doesn't keep the generated skill folders: `.claude/skills/` for Claude Code, and `.agents/skills/` for Codex, Gemini, Cursor, and GitHub Copilot.
...
Claude Code may create `.claude/settings.local.json` for permissions on one computer. discern doesn't seed or track it, and discern's `.gitignore` rules keep it out of Git.
...
GitHub Copilot can use the same `.mcp.json` file. Claude Code and Copilot share one writer for this entry, so the `discern` server is byte-identical whichever tool wires it first.
...
`discern refresh` adds or updates discern's hooks and leaves other hooks and permission rules unchanged. It also adds `discern` to `enabledMcpjsonServers`. Once you trust the workspace, Claude Code can start that named local server without another server-approval prompt. Claude Code still applies its normal permissions when the server calls a tool.
...
`EnterWorktree` moves Claude Code's shell and instruction context, but a stdio MCP process keeps the directory it started in. `discern_start` points discern's running MCP server at the new worktree. A native directory move doesn't move a generic server.
...
The writable root includes the main checkout's folder name. discern only ever adds its root to the list, and never removes one. So in a clone under a different folder name, `discern refresh` adds a second root for that folder, and the first stays. Review that change before you commit it. Each root stays narrower than the parent directory.

discern doesn't set an MCP `cwd`, and refresh removes one from discern's entry. Codex starts the project's server from the project root by default, and overriding that can detach the server from the project's `discern.toml`.

discern doesn't set `sandbox_mode`, `approval_policy`, `approvals_reviewer`, model settings, network access, `required = true`, MCP tool lists, or tool approval modes. Those are security choices for you or your project. Restricting tools here could also block the command-line fallback or future discern tools.
...
`discern refresh` re-seeds this hook idempotently and keeps unrelated settings and hook groups in the same file. The hook reruns worktree setup when a Codex session opens or resumes inside a discern worktree. Codex holds committed hooks until you trust the hook's hash, so the command doesn't run before then.
...
discern adds `version` and `name` only when they're missing. A new file uses the project's configured name, or the name discern already shows for the project. An existing Codex file keeps its values. `discern refresh` restores missing setup and cleanup scripts, and keeps scripts the project has customized. discern puts its marker comment at the top of the file.
...
Codex loads this rules file after you trust the project. Each rule allows a command that begins with the listed words, wherever Codex runs it. discern writes its own file and leaves project files such as `.codex/rules/default.rules` unchanged.
...
Codex ignores the project's `.codex/` settings until its user-level configuration marks the project's absolute path as trusted: `projects."<absolute-project-path>".trust_level = "trusted"` in `~/.codex/config.toml`. That decision lives outside the repository, and discern can't make it for you. Codex also asks you to approve each committed hook's hash before the hook runs, and `--dangerously-bypass-hook-trust` skips that approval. After you change the project settings or trust them, start a new session or restart Codex.

`discern_start` points the running discern MCP server at the new worktree, but it can't move Codex's shell there. The writable root lets Codex work inside the worktree folder, and the rules allow only commands that begin with `git add` or `git commit`, wherever the trusted session runs them. So drive the worktree explicitly: prefix every shell command with `cd <path> &&`, and pass `path` to every discern tool. Your edits and the gate then use the same worktree.

If a Codex session starts inside a worktree that's later removed, Codex can block the next message with "Current working directory missing". This happens even after `discern_accept` removes the worktree and points the MCP server back at the main checkout, because the Codex chat process remembers the deleted directory it opened in. Once Codex blocks the conversation, you can't recover it from inside the chat. Start a new Codex session from the main checkout.
...
Gemini's safe mode ignores the project's `.gemini/settings.json` until Gemini trusts the folder. That trust lives outside the repository. Before trust, Gemini still reads `GEMINI.md` as instructions, but it doesn't load the committed MCP server and hooks. To load them, trust the folder, or choose a bypass such as `--skip-trust` or `GEMINI_CLI_TRUST_WORKSPACE=true`.
...
After you change the configured agent set or upgrade the project, run `discern refresh`. It keeps the committed settings in line with discern's provider registry.
...
A Local session stays rooted at its checkout, and `discern start` creates a worktree beside it, outside that workspace. **External File Protection** can then pause each write there for your approval. Cursor doesn't tell the agent process about that pause.
...
Cursor requires `type: "stdio"`. discern writes this entry with the same writer as Claude Code's and GitHub Copilot's ([ADR 0074](https://discern.sh/docs/decisions/0074-co-owned-mcp-json)). Unlike theirs, it lives in `.cursor/mcp.json`, passes `--strict-tool-calls`, and sets no `timeout`.

#### Logbook identity

Cursor's MCP client declares the name `cursor-vscode`. discern's agent catalog recognizes that exact name, and has no `cursor-*` prefix rule. New calls that carry only that name get a Cursor `mcp-client` signal in the [logbook](logbook.md#possible-agent-identity-signals).
...
The editor and the CLI read the same `.cursor/mcp.json`. discern doesn't use advisory MCP client names to choose behavior, so the generated entry selects the shortest verified profile. `discern_await` makes 45-second calls, which leaves 15 seconds to deliver the result. When the condition hasn't been met, the result carries a 15-character continuation handle and a `--resume` command that keep the original watch. Continue until the condition holds, you stop the watch, or the task no longer needs it.

A gate call can take longer than the Cursor Agent CLI's 60-second limit. Run `discern done --markdown` in a shell instead. Use `discern prepare --markdown` or `discern test --markdown` there for those stages of the gate.
...
Cursor's workspace trust controls the committed `.cursor/` configuration. The MCP server can also ask for approval of each tool on first use. In a headless run, `--approve-mcps` skips the MCP approval prompt, but it doesn't replace workspace trust.
...
discern doesn't currently detect a Copilot installation that runs only inside an editor, an integrated development environment (IDE). `discern setup` finds GitHub Copilot only when the `copilot` terminal CLI is on `PATH`. For an editor-only install, add `copilot` to `[project].agents` yourself, then run `discern refresh`:
...
Copilot supports both `.mcp.json` at the repository root and `.github/mcp.json`. discern uses the root file because Claude Code reads the same server entry there, so both tools can share it. discern doesn't write the Claude Code approval setting for Copilot, which relies on folder trust instead.
...
Copilot ignores the committed configuration until you trust the folder. The trust grant lives outside the repository, in `trustedFolders` in `~/.copilot/config.json`. For unattended runs, discern's provider registry names `--allow-all-tools` and `--allow-all-paths` as the headless bypasses.

Copilot can move a running session with `/cwd`, and create and enter a native worktree with `/worktree`. It still exposes no worktree create and remove hook for discern to drive. discern runs its own worktree lifecycle through `discern start`, `discern update`, and `discern accept`.

In interactive mode, Copilot's `sessionStart` hook can fire once per prompt. `discern worktree ensure` is idempotent, so repeated calls leave the same result.
...
discern uses the operating system's secure random source, through WebCrypto, for identifiers, nonces, and key material. This is separate from the deterministic worktree identities above. [Files and ownership](files-and-ownership.md#runtime-state-inside-git) lists the local records, including where discern keeps its key.

If you're extending or contributing to discern, [secure entropy in discern's project map](https://github.com/discern-sh/discern/blob/main/project/map/00-orientation/secure-entropy.md) covers the implementation and its security boundaries.
