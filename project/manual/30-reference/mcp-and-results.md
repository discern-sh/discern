...
| `discern_done`              | `dry_run`, `ci`, `rerun`, `met`, `unmet`, `path`, `standalone`, `recover`, `context`, `policy_base`, `retain_checkout` |
| `discern_update`            | `from`, `dry_run`, `path`                                                                                              |
| `discern_await`             | `green`, `landed`, `trunk_moved`, `resume`, `timeout`, `path`                                                          |
| `discern_accept`            | `action`, `reason`, `confirmation`, `recover`, `dry_run`, `confirmed`, `variance`, `approve_standard`, `path`          |
| `discern_test`              | `path`                                                                                                                 |
...
For checkout recovery, `discern_done` accepts `recover` with an environment id. It returns the owned checkout and settles its reservation without validation or landing. For emergency integration, `discern_accept` accepts `action: "emergency"` and a `reason`. Review the exact plan before supplying fresh owner approval with `confirmed` and its `confirmation` token. The exception has its own record type and cannot serve as passing Proof. `recover` resumes an interrupted emergency by landing id.
