...
([`tests/agent_agnostic_test.ts`](../../../tests/agent_agnostic_test.ts))
forbids any `.claude` path in `src/engine/**`; the `.claude/worktrees`
convention and the coupling to Claude Code's hook JSON shape are
Claude-Code-specific, so they sit beside `src/lib/skills.ts` (which already owns
`.claude/skills` materialization). The engine keeps a convention-free
