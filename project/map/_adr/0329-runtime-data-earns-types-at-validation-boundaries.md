...
The setup config document adds a compatibility constraint. Its published editor schema is a strict snapshot that catches misspelled fields, while an older runtime intentionally ignores optional fields introduced by a newer release in the same schema major. Copying that document shape into a second permissive validator would trade one trust jump for a new drift risk.
...
Schemas live with the canonical shape or the boundary they describe, and inferred runtime types derive from them. The config-document runtime schema derives from the same strict `configDocSchema` that supplies its live TypeScript type and published editor schema. It projects away only keys that canonical schema identifies as unknown, including nested keys, then validates every known field. One subsequent version check rejects an unsupported major. The runtime does not maintain a second known-field list and does not apply the strict editor snapshot directly.
...
- Setup config documents preserve same-major forward compatibility. An old runtime ignores a new optional field while still rejecting a wrong type for every field it knows; the strict published schema can continue flagging unknown fields for authors using that release's snapshot.
...
- **Use the strict editor schema at runtime for setup documents.** Rejected because it would turn an additive optional same-major field into a compatibility break.
- **Write a permissive config validator by hand.** Rejected because its known-field list could drift from the live config type and generated schema.
