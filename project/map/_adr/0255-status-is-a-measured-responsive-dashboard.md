...
> **Amendments.**
>
> - **[ADR 0279](0279-external-terminal-rendering-crosses-one-process-boundary.md) — presentation implementation:** the typed status result, precedence, and complete identity policy survive. The published package's lossless Fleet mode and Components now own responsive terminal geometry and reusable frames; Discern supplies safe product facts and explicit time/capability context. Proof is the current product term for the Gate evidence historically called a receipt below. Machine compatibility fields retain their declared names.
> - **[ADR 0281](0281-main-fleet-status-is-a-decision-brief.md) — decision brief:** the current human projection leads with main checkout state and task labels rather than generated Git identity. Live, stale, and uncommitted lifecycle states precede branch lag. Shared-file and ADR-number collisions are landing relationships, never row states. Complete Git identity moves to expanded evidence. Every visible block uses a published CLI Component, while drift keeps its arrow-and-count vocabulary with design-system semantic color. These rules supersede the earlier fleet-first, collision-precedence, and identity-leading presentation paragraphs below; the typed result remains unchanged.
> - **[ADR 0306](0306-status-defaults-to-a-bounded-orientation-projection.md) — bounded orientation:** status now defaults to a bounded orientation projection — the default structured projection caps fleet samples and moves landing history out of orientation; full mode restores the complete collections.
