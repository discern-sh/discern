...
> **Self-hosting amendment (2026-07-13):** The product decision that fresh
> projects default to `map/` stands. This repository no longer adopts that
> default: its map resolves to `project/map/` alongside every other ongoing
> configurable authored source, exercising the existing path override.
...
moves out from under its project. This repo originally adopted the new default
by moving its tree wholesale (`git mv docs/ map/`), historical ADR content
untouched; the self-hosting amendment above now points it at `project/map/`. MCP
tools rename in lockstep, and the existing parity guards force the satellites.
The receipt naming is unchanged (the July 10, 2026 decision stands), and the
"harness" retirement proceeds per-context as already planned.
...
- The two config-table renames are the deepest changes — `${docs.dir}` is
  ...
  map's ownership becomes physical. Fresh projects place discern's output at
  `map/`; this repository exercises `[map].dir` at `project/map/`; the host
