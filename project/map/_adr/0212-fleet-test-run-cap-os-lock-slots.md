...
The explicit *no*s: no daemon or polling loop against the logbook; no per-job or weighted slots; no scope-gate enrolment; no machine-wide (cross-repository) cap; no queue fairness — flock wakeups are unordered, so under heavy contention a waiter can be overtaken; the cap bounds concurrency, not arrival order.
...
- **A lease/heartbeat file protocol instead of OS locks.** Rejected: it reintroduces stale-state reclamation, timing heuristics, and a wedged-fleet failure mode — everything the kernel already does for free.
