...
- **A report file** under `<git-common-dir>/discern/crash/` (a common-scope Git-admin-state registry entry, beside the logbook — never inside the project tree), named by sortable timestamp, process id, and a filesystem-generated unique suffix. It carries the discern version, Deno runtime, platform, verb, error, and full stack. The newest 20 are kept; outside a repository the report falls back to a system temp file. Writing is best-effort and never throws — a reporting failure must not mask the crash.
