...
2. **It forced the binary's artifacts to cohabit with your files.** `.discern/skills/` held **bundled** skills (generated, gitignored, re-published) _and_ **authored** skills (project-owned, tracked) in one directory — a part-tracked/part-ignored mess that silently de-tracked authored skills on upgrade. (The concrete trigger: a repo with hand-authored skills under `.discern/skills/` would lose them by following the upgrade guide literally.)
