...
> `accept`, `scopes` → `impact` where it names the verb; the decision and
...
> builders live in `src/engine/gate/plan.ts`,
> `src/engine/gate/standard_plan.ts`, and the worktree lifecycle. The decision
> stands; only the file locations moved (the cited `engine/plan/*` paths below
...
([`src/lib/fs_plan.ts`](../../../src/lib/fs_plan.ts)) — and a thin executor
applies it, with rendering kept separate
([`src/lib/plan_view.ts`](../../../src/lib/plan_view.ts)) so planning stays
pure. That one split is why those verbs get `--dry-run`, a review screen, and
...
- `accept`, `worktreeSetup`, `worktreeTeardown`, `worktreePrune`
  (`worktree/lifecycle.ts`) were imperative effect-chains. The scary, stateful
  ones (prune's GC; the teardown / checkout or fast-forward dance in `accept`)
...
1. **No `--dry-run` on the verbs that most need it.** You could not preview a
   acceptance or a prune-GC pass before it mutated git and destroyed resources.
...
`setup-step` / `env` / `refresh` / `standard`, with a `StepDisposition` of `run`
...
commands; an `AcceptPlan` carries the diagnosed git state) so its executor stays
type-safe, and projects that plan to a common `EnginePlan` for presentation. A
gate plan and a worktree plan are different types sharing one renderer.
...
- the prune-GC reclaim decision — `classifyOrphans` in
  ...
  tested directly;
- standard derivation — `buildStandardPlan` in
  [`gate/standard_plan.ts`](../../../src/engine/gate/standard_plan.ts).
...
existing `done --json` tests stay green unchanged.

## Consequences

- **`--dry-run` works on every effectful verb** — `done`, `accept`, `worktree`
  (setup), `worktree teardown`, `worktree prune`, and `standards` — free,
  because the plan is computed before any effect. Each also accepts
  `--dry-run --json` (the plan as JSON) and, when applied, `--json` (a
  serialization of plan + results). The pure-query verbs `identity` and
  `impact`, and the thin `prepare`/`test` gate variants, were left alone — they
  have no apply step to plan.
- **The planners are unit-tested without a subprocess.** `gate_plan_test.ts`,
  `worktree_plan_test.ts`, and `standard_plan_test.ts` exercise the whole "what
...
- **One intended human-output change.** `worktree prune --dry-run` now renders
  ...
  point). Every other existing human string and the `done --json`,
...
- **One genuinely safe ordering shift.** Acceptance now creates the worktree's
...
- **One universal engine plan type.** Rejected: a gate plan (job groups) and a
  accept plan (diagnosed git state) carry genuinely different data. Letting each
  verb keep its typed plan and project to a common `EnginePlan` for rendering
  keeps the executors type-safe without a lowest-common-denominator union.
...
This generalizes the installer's [`fs_plan`](../../../src/lib/fs_plan.ts)
pattern across the engine seam, preserves
