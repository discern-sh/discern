...
> pointers use `ratchets` → `standards`, `graduate` → `accept`, `docs` → `map`
> where it names the command, config, or tree, the retired product-category
> wording → `discern`, the gate, or the bar; the decision and reasoning are
> unchanged.

**Status**: accepted. **Extends [ADR 0003](0003-named-metric-standards.md)**
(named metric standards) — the standard mechanism is unchanged; this adds an
optional denominator so the _measured value_ can be a rate.

## Context

ADR 0003 made any only-ever-improve number eligible for a standard: a `run`
emits `DISCERN_METRIC <name> <number>`, and the engine holds it against a
`limit` compared to `main`. The number it holds is a raw scalar.
...
ceiling — i.e. to loosen the standard, the one thing it exists to forbid.

We hit this dogfooding discern's own prose standard. The Vale alert count sat at
1703 with the ceiling locked one above it (1704); discern simultaneously _tells_
agents to write ADRs and keep docs current, which drops new prose straight into
the scanned corpus. Every doc-writing branch breached the ceiling, and agents
"resolved" it by bumping the limit and rationalising the resulting
loosened-vs-main failure as a transient that resolves on accept. The metric was
**extensive** (scales with size); a standard is only honest on an **intensive**
one (a rate or proportion, size divided out). The fix is to let a standard
_hold_ a rate. A contributing cause was the guidance itself: "tighten the limit
to lock the gain in" is sound for a ratio but, applied to a count, manufactures
exactly the zero-headroom ceiling that forces the next branch to loosen.

## Decision

Add two optional keys to a `[standards.<name>]` table. They change only the
...
- **A built-in extent** (`{ files | lines | words | bytes = <git pathspec> }`):
  ...
  neutrality, and it is the difference between "add one line to hold a rate" and
...
how rate standards are _taught_:

- The shipped guidance leads with a test, not a term: _"does this number grow as
  the project grows, even when nothing got worse? then hold a rate."_ It drops
  the "lock the gain in" advice that built zero-margin ceilings, and states that
  a limit loosened versus `main` is the regression the standard exists to catch,
  never a baseline to reset.
- When a raw-count ceiling is breached, `discern standards` points at the fix in
...
- A growing tree never breaches a normalized standard on its own; only a real
  quality regression (a worse-than-average rate) does. The recurring
  loosened-vs-`main` churn on the prose standard disappears.
- Discern's own prose standard converts to `per = { words = "map/**" }`,
  `scale = 1000`, tracking ~18.7 alerts per 1,000 words instead of a raw 1703.
- **Converting an existing standard from count to rate changes the limit's
  ...
  legitimate one-time change that resolves on accept (it did not for the prose
  conversion: 19 < 1704).
...
- **Standard an arbitrary expression over emitted metrics**
