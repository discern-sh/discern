...
The reclaim runs the same teardown lifecycle acceptance uses. External resources die through their ledger commands, then the registration goes — never a raw `git worktree remove`. A failed destroy refuses that candidate's reclaim: the checkout stays and keeps owning its resources. The ref survives as the recovery guarantee and needs no new cleanup machinery. Once the train lands, the existing landed-branch prune deletes it as ordinary fully merged history.
