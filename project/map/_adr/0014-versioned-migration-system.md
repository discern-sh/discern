...
>
> **Install corpus amendment (2026-09-18):** the committed corpus promised in decision 6 lives under [`tests/fixtures/installs/`](../../../tests/fixtures/installs/README.md): one Git-carried `discern setup begin` capture per past schema, starting with the schema-1 release candidate, with its source commit and reproduction command in each manifest. [`install_corpus_test.ts`](../../../tests/install_corpus_test.ts) upgrades every capture through the production chain and requires byte equality with a fresh installation, except for an explicit allowance registry that asserts each difference's shape (setup provenance and adoption state in `discern.toml`). Because every config writer passes the canonical formatter, the shape-level convergence below is held at the byte level; a seed that cannot converge byte for byte needs a reasoned registry entry, not a looser comparison. `SCHEMA_VERSION` may not exceed the newest captured schema plus one, so a schema cannot advance without freezing its predecessor.
