...
- **No size bounds** (`max_files`, diff-line caps). Counts give the *appearance* of policy — a three-file change can rewrite the release pipeline. Grants bind to meaning (scopes), never magnitude.
...
- The bypass incentive inverts: the cheapest way to land a small delegated change is now *through* the worktree flow, where it is receipted and logged — not around it.
