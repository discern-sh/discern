...
> **Dependency-pin amendment (2026-07-28):** Discern now pins `jsr:@discern-sh/design-system@0.9.0` in `deno.json` and `deno.lock`. The `0.1.1` line below records the initial external cut-over. Exact immutable consumption and external package ownership remain the current decisions.
