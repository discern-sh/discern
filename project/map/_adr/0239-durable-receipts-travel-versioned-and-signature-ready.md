# ADR 0239: The durable receipt travels as a versioned, signature-ready record
...
**The format is published as its own contract.** `schema/discern-receipt-note.schema.json`, served at its `$id`, joins `PUBLIC_SCHEMA_PUBLICATIONS` under the additive result-output policy: the artifact is generated from the same Zod source as the runtime schema, permits unknown fields as the reader does, and a breaking change to the note format is a new major path — mirrored in-band by a new `format` value that old readers report as `unsupported`. The receipt object itself becomes the named `DiscernReceipt` definition inside the results schema, referenced by every contract that carries one instead of inlined per contract. It does not get a second standalone `$id`: the receipt's identity inside results is owned by the results contract, and the note schema embeds its own copy so a note consumer reads one self-contained definition.
