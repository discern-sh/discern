...
Reading and validating are two policies over ONE parser. `parseFrontmatter` reads leniently — no reader of a map can ever lose a document to a metadata mistake — while `validateFrontmatter` applies the strict schema and an architectural test walks the live map (minus `_private`) so a typo'd key, duplicate key, out-of-bounds value, broken fence, duplicate sibling `order`, or unsafe redirect claim fails `discern done`. `SKILL.md` blocks read the same scanner.
