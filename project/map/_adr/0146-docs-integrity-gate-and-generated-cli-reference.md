...
4. **Audience boundary**: no published page (a published tier admitted by `isPublicDoc`, [ADR 0140](0140-validated-frontmatter-and-the-publish-predicate.md)) may link into `_internal/` or `_private/`. Citing `_adr/` stays legal — decisions publish ([ADR 0141](0141-adr-citations-strip-at-render.md)).

The guarded corpus is the CURRENT map — every doc outside `_`-prefixed subtrees, root docs included. The `_` trees are exempt by design: ADRs are dated records, `_internal`/`_private` carry no currency contract.
