...
- **The maintainer tier (`_private/maintainer/`), beside the strategy corpus it feeds.** The first cut landed there; rejected on review. That tier holds launch-phase working notes, not durable inventories, and it sits outside even the Vale pass — the one mechanical prose check a page agents quote from should keep.
