...
- No change to any default bound, and no mention on public surfaces: the variable is registered as internal documentation, recorded on the contributor experimental-behaviors page only.
...
- `timeout_basis` gains one value in the published result schema. That vocabulary outlives an eventual experiment removal or ripens with its promotion; removing an enum member later is the breaking edge of this trial.
