...
- **The carry-forward belongs to the mutator, not to accept.** Scope classification is path-based; it cannot see that only `[standards]` limits moved _inside_ `discern.toml` (the same file also holds `[capabilities]`, which the gate does read). Only the operation that authored the commit has that knowledge, so the vouch is re-stamped where it is _known_ safe — never inferred from a diff after the fact.
