...
- **The leakage guard battery**, strongest first:
  ...
  5. **Dogfooding on non-defaults.** This repo permanently points every ongoing
     configurable authored source beneath `project/`. A registry-driven guard
     auto-enrols new keyed sources, so any hard-coded default or scattered path
     diverges from the repo agents can see — leaks become loud.
