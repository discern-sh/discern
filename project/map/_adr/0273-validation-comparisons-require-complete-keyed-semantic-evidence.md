...
- every index entry's mode, object id, stage, and raw path bytes from `git ls-files --stage -z`, including conflict stages;
- each tracked worktree entry that differs from the index, including deletion, type, executable mode, symlink target, and binary bytes;
- every untracked entry that Git does not ignore, with the same filesystem distinctions; and
- the working commit of each initialized submodule.

The index manifest deliberately excludes physical index stat-cache and extension bytes. `ls-files` resolves split-index storage and expands sparse representations to semantic entries, so an index refresh does not move the identity while a staged object, mode, stage, or path does. A dirty submodule makes the snapshot incomplete; version 1 does not recursively publish an identity for arbitrary nested dirt.

A random 32-byte HMAC-SHA-256 key lives at the registered repository-common Git-admin path `discern/validation-hmac-key`, mode `0600`. Component and combined state digests, config identity, worktree-setup identity, and job-definition digests are domain-separated keyed hashes. Logbook lines retain only opaque digests, counts, byte totals, capture timing, completeness, and categorical failure reasons. They contain no manifest, path, content, command, config value, environment value, or reusable plain hash. Job ids, stages, kinds, normalized outcomes, execution mode, concurrent-sibling flags, and writer version remain ordinary bounded metadata.

Capture has hard ceilings of 20,000 path observations, 64 MiB of index/content bytes, and 5 seconds. The time ceiling is passed into each Git probe, which kills the process at the remaining deadline; filesystem reads are size-checked before allocation and time-checked after completion. A committed benchmark fixture with 250 one-KiB files must remain below one tenth of the path limit, one hundredth of the byte limit, and the time limit. The limits leave ample room over that representative small-project shape without letting a Logbook append become an unbounded scan.

Any key, Git, filesystem, path-decoding, submodule, or budget uncertainty sets `complete: false`, records a categorical reason, and omits the combined digest. Readers may compare only complete state and complete execution envelopes of the same validation-evidence version. Mode, writer, config/setup digests, test job definitions, and concurrent-sibling flags are part of the comparison key. Legacy identity is version-prefixed separately. Incomplete, legacy, and current evidence never blend.
...
- Large, unreadable, racy, or dirty-submodule trees still run their configured validation normally. Their Logbook event is honest about incompleteness and cannot support a same-input claim.
...
- **Recursively fingerprint dirty submodules.** Rejected for version 1 because it makes cost and nested ignore/config semantics unbounded. Dirty submodules instead fail comparability explicitly.
