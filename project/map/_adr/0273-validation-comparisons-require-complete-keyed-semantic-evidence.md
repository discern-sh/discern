...
Repository capture has hard ceilings of 20,000 path observations, 64 MiB of Git output plus index/content bytes, and 5 seconds. One capture authority passes the remaining time and combined stdout/stderr allowance into every Git probe; crossing either kills the process and closes its capture streams before generic buffering can exceed the boundary. Filesystem reads are size-checked before allocation and time-checked after completion. The execution envelope has its own ceilings of 1,000 selected jobs and 1 MiB of canonical config, setup, and job-definition input, size-checked before serialization. A committed benchmark fixture with 250 one-KiB files must remain below one tenth of the path limit, one hundredth of the byte limit, and the time limit.
