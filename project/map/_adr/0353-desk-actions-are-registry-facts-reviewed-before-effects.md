...
Every material effect crosses a review composition in [`view.ts`](../../../src/engine/desk/view.ts). Lifecycle actions render the read-only plan produced by their authoritative core through Command, Procedure, ProcedureStep, ExpectedResult, and DestructiveActionNotice Components where those contracts fit. The composition also renders Keeps, Changes, Removes, and Recoverable. Apply reads current state again after confirmation; a menu predicate never authorizes an effect. All mutating confirmations default to No, and Drop keeps its typed branch challenge before discarding refused work.
