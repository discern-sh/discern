...
- **Classify questions in the memberships instead of the vocabulary.** Rejected: how violations arise is a fact about the question, and the precondition for a membership decision — recording it on the member would let two memberships disagree.
- **Mark reviews guarded whenever a shipped seed exists for the question.** Rejected: a seed no entry enables protects nothing; the mark must mean this project's boundary.
...
- **Let a graduation recommendation name an unshipped built-in id.** Rejected: the coach would be recommending content that does not exist; routes bind to canonical questions instead.
