...
- Agent availability is the intersection of the selected checkout's committed `[guidance].agents` and a live scan of the provider registry's known binaries on `PATH`. Neither side is a fallback: detected-only agents stay hidden, and configured-but-unavailable agents cannot be selected.
