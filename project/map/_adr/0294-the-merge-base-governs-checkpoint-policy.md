...
A branch edits its own `discern.toml`. If the working tree's `[checkpoints]` tables governed the branch's own gate, deleting or weakening a checkpoint would be a one-line self-exemption — the same hole the standards never-loosen check closes for numeric limits. Unlike a limit, a criterion is prose: two versions of judgment text are not orderable, so no "never loosen" comparison can exist for checkpoints. Reading the live trunk tip instead would make gate outcomes change under an effort's feet whenever the trunk moved.
...
- Removing, weakening, or adding a checkpoint on a branch has no effect on that branch's own gate; the edit takes effect for other efforts only after it lands. `discern.toml` is an unscoped path, so such an edit already requires owner review at acceptance.
...
- An agent cannot relax its own checkpoint obligations from inside an effort, and an owner reviewing a landing sees any policy edit in the diff of an unscoped file.
