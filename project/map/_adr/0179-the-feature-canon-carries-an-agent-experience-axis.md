...
`hints` lists registered hint ids the node's account leans on. The enrolment guard fails a citation of an id the hint registry no longer carries — and that is all. The hint corpus is deliberately **not** an enrolment set: no hint demands a citation, and no completeness check runs. This departs from the house full-enrolment discipline ([ADR 0051](0051-canonical-set-parity.md)) on purpose. The hint corpus is an order of magnitude larger than any enrolled set, most entries are operational detail beneath the canon's resolution, and a citation is an evidence pointer ("this account is implemented by that hint"), not a coverage contract. Guards extend symmetrically elsewhere: agent accounts obey the same sentence-completeness and live-verb command-mention checks as `what` and `why`, and a citation may only ride an account.
