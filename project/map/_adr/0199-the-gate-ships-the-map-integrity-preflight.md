...
`discern done` runs a **map & guidance integrity preflight** in every project — a blocking fail-fast precondition (`failed_stage: "map_integrity"`) beside the currency checks, with no warn tier and no configuration knob, exactly like the other artifact preflights. The core is pure — `checkDocsIntegrity(root, config, cli) → findings` in `src/lib/map_integrity.ts` — and one diagnostic groups every finding as `file:line` under its rule with the rule's remedy stated once.
...
1. **Links and anchors** — every intra-map link resolves; every fragment names a heading id the shared renderer mints. Root-relative targets (`/x`) are treated as external, like scheme'd URLs: a map published to a site legitimately uses site-absolute routes the checkout cannot resolve.
...
- Citation checking is deliberately blind to unbackticked prose; the convention every bundled surface teaches (backtick the skill name) is now also what makes a recommendation checkable.
