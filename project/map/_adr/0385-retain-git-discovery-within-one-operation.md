...
The scope ends when its operation returns, so nothing survives between calls of a long-lived process. Newly held exclusion, a publication's FIFO wait and environment restoration mark it stale, matching the boundaries the completion store already treats as fresh observations; the shared git runner does the same after any `git worktree`, `git init`, `git submodule` or `git clone` invocation, so no call site can forget. A stale checkout re-observes its administration directories with one git process before answering again. Registered paths, work-tree positions and pinned objects are pure functions of those directories, so they survive only when the fresh observation is byte-identical to the retained one. Outside a scope every query runs fresh with its single argument list, exactly as before.
...
- Retaining across lock acquisition without re-observing would remove a further 7% of the journey's spawns, at the cost of departing from the completion store's documented boundary that storage is resolved inside each held lock.
- Discarding every derived answer on each boundary, rather than re-validating it against the fresh directory observation, left the registered-path queries almost untouched in the traced journey.
