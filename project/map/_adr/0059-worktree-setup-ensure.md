...
> pointers use `integrate` → `update`; the decision and reasoning are unchanged.

**Status**: accepted. Realizes the deferred "Part B" of
[ADR 0055](0055-update-verb.md), and mirrors the resource `create`/`ensure`
...
`discern update`: it brings `main` into a branch and re-materializes the
...
late, slow failure update exists to prevent for agent files. ADR 0055 deferred
"re-run the worktree's setup after a merge" as **Part B**.

The naive Part B — blindly re-run **all** setup steps on every update — was
...
on every update, its "already exists" non-zero exit turning a good worktree red.
...
- **`discern update`** — after the merge + the agent-file refresh. This is the
  motivating case: a merge that changed a lockfile is followed by a reinstall,
  with no manual step.
...
- on **re-entry or update**, an `ensure` failure is **non-fatal but recorded** —
  a failed step plus a warning. It never undoes a completed merge or breaks
  session start over a convergence hiccup; the gate is the backstop. This
  mirrors how update already treats its post-merge refresh (recorded as
  `failed`, merge kept).
...
**`ensure` runs at session start too**, not only at creation and update, for
...
`setup-ensure` step kind lets the setup and update plans — and their `--dry-run`
/ `--json` — distinguish the two buckets.

## Consequences

- **The staleness gap closes for the user-owned environment.** A merge that
  changes a lockfile, followed by `discern update`, leaves dependencies current
  with no manual reinstall — the same gap update already closed for the
  discern-owned agent files, now closed for the environment.
...
- **No silent middle ground.** A failed `ensure` is either fatal (creation) or a
  recorded failed step (re-entry / update) — never swallowed.

## Alternatives considered

- **Blindly re-run all `setup.steps` on every update** (the naive Part B).
...
- **Run `ensure` only at creation + update, not session start.** Rejected — it
  breaks parity with resource `ensure`, and session start is the natural
