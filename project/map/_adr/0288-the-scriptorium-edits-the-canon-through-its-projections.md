...
- **Write-back is deliberately narrow.** A save replaces exactly one string-literal prose field, formats via the repository's formatter, re-renders in a fresh subprocess, rewrites the committed pages to the generator's exact bytes, and runs the registry's guard files from its `canonical_sets.ts` roster. Any red step restores every written byte. Field semantics compile `satisfies Record<keyof …>` against the registry interfaces, so a new field fails the studio's typecheck until classified.
...
- Structural operations — adding, retiring, reordering entries, list-field pickers, a commit composer — are deliberately out of scope; agent-mediated editing remains the tool for campaigns.
