...
Checkpoints make the evidence boundary especially important. A coding agent may judge a checkpoint question, and Proof carries that declared conclusion beside machine results. The machine verdict remains model-free because discern verifies the declaration's presence and binding without verifying its semantic truth. An unqualified “no model in the verdict” would collapse those evidence kinds whenever a model-backed coding agent supplies the judgment. Shipped checkpoint thresholds and configured `when` commands likewise require Standard-limit and external-command claims to state their real scope.
...
- The default scope is discern-owned code and effects. Configured jobs, checkpoint `when` commands, resources, hooks, coding-agent clients, and coding-agent host permissions remain outside a claim unless a record includes them. Agent-authored declarations are external judgment recorded by discern. Qualifications belong on the record rather than in corrective footnotes elsewhere.
- Evidence kinds remain distinct in boundary wording. Machine results are verified, checkpoint conclusions are declared, and variances are authorized. “No model inside” means discern performs no inference; it does not erase a coding agent's judgment from Proof.
...
- A statement can be strong without being indefensible. The no-network record says the public binary has no network permission and places project jobs and checkpoint `when` commands outside that guarantee. The Standard-limit record covers measured ratchets without pretending discern ships no configurable checkpoint thresholds.
