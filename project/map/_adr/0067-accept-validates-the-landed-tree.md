...
> **Amendments.**
>
> - **Vocabulary ([ADR 0120](0120-launch-verb-canon.md), [ADR 0245](0245-receipt-renamed-to-proof.md)):** current spellings are `done` (formerly `finish`), `accept` (formerly `graduate`), and `update` (formerly `integrate`); the gate-pass artifact became the receipt and is now the **proof** — receipt-family terms below read as their proof-family successors; the decisions below are unchanged.
> - **[ADR 0110](0110-the-landing-model.md) — landing target:** `accept --to trunk` below is now plain `accept` — the configurable destination was removed and the trunk is the single landing target; the proof-fast-pathed validation decided here stands unchanged.
> - **[ADR 0152](0152-slow-workflows-prove-write-authority-first.md) — write authority:** proof I/O remains best-effort against failures that arise after validation starts, but `done` now proves its Git-admin write authority before project jobs run; a known denial is a fail-fast gate precondition, not a green run without a proof that acceptance has to repeat.
> - **[ADR 0165](0165-git-admin-state-namespaced-by-lifetime.md) — marker path:** the worktree-local proof marker lives beneath Discern's Git-admin `discern/` namespace, and its filename followed the proof rename (`src/engine/gate/proof.ts` is the live authority); its identity and lifetime are unchanged.
> - **[ADR 0313](0313-setup-completion-and-acceptance-bind-one-final-proof.md) — setup landing:** the setup-specific acceptance path now uses the same Proof inspection and exact validated-commit rule. It refuses incomplete Proof instead of rerunning the Gate implicitly; a moved trunk earns new Proof on the setup branch before landing.
