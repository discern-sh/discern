...
Project Scripts resolve direct regular-file names literally and always run from the project root. They inherit ordinary process values after every ambient or supplied `DISCERN_*` name is removed. discern then exports four names: absolute `DISCERN_ROOT`, absolute `DISCERN_TOML`, absolute `DISCERN_SCRIPTS_DIR`, and `DISCERN_TRUNK`. No lock lease, identity field, alias, or internal diagnostic variable crosses that boundary. Unknown, non-executable, and non-command paths each retain a registered JSON refusal when machine mode was selected before the name.
