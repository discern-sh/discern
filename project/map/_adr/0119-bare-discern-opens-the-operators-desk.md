...
- **The gate is the shared interaction policy.** The desk shows when `canPrompt` (`src/lib/prompts.ts`) holds — stdin AND stdout are terminals, `--plain` is absent, and the process is not under an enabled `CI` environment — and never otherwise. There is deliberately NO detection of agent or vendor markers. `discern desk` invoked without a TTY (or with `--json`) refuses with a structured `interactive_only` result pointing at `status`.
