...
> Amended by [ADR 0391](0391-landings-compose-a-moved-trunk-in-an-integration-worktree.md): landings now serialize on a dedicated acceptance boundary for that whole span, while the common publication boundary joins per short transactional phase — the transaction's atomicity against other landings is unchanged, and a composed landing's long check no longer starves sibling completion publications.
