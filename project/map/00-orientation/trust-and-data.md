...
A green landing records its structured receipt under the local Git ref `refs/notes/discern`. The note is a self-describing record: it names its own published format, the full commit id it vouches for, and the receipt — branch, diffstat, standards result, one-line summary, and Markdown page. Reserved fields hold room for a signing identity; while nothing signs, their absence means unsigned, and older bare notes still read. It is authored by `discern <done@discern.sh>` unless `DISCERN_NO_ATTRIBUTION` asks Git to use the repository identity instead. Delete one with `git notes --ref=discern remove <commit>`, or delete the local channel with `git update-ref -d refs/notes/discern`.
