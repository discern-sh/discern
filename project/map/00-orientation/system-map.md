...
- **Persistent state lives with the repository.** `discern.toml` plus the git repository itself: branches, linked worktrees in a sibling `<repo>.worktrees/` folder by default (configurable via `[worktree].root`), durable Proof notes under `refs/notes/discern`, and [runtime records inside the Git directory](../70-reference/artifact-ownership.md#runtime-state-inside-git) — the resource ledger, the Logbook, and interruption-recovery journals. No manifest and no product database of discern's own. [Per-worktree resources](../30-worktrees/the-resources.md) a project declares (a database, a dev site) are provisioned by the project's own commands and tracked in the ledger so teardown can find them.
