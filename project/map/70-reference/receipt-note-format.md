...
- `format` — the published schema URL, carried in the bytes. A note may be read by a different discern release than the one that wrote it, so the record names its own contract. The `/v1/` path segment is the compatibility major.
...
3. Treat an unrecognized `format` value as unsupported, never as "no receipt". `discern status` reports that case as `data.landed_receipt_unsupported`, naming the format.
