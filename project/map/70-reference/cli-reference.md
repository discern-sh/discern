...
| `--theme <theme>` | Set the terminal theme. `auto` senses the background; `light` and `dark` force that variant. Default: `auto`. `--no-color` and `NO_COLOR` still disable colour. |
