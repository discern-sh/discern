...
| `paths`             | string[]           | —       | Selector globs in the scope dialect: prefix, standard glob, suffix, segment, or exact path. Use either `scope` or `paths`. Supports registered path references.                                                        |
...
| `exclude_paths`     | string[]           | —       | Globs removed after selection and before every predicate, subject, evidence, and `when`. Supports registered path references.                                                                                          |
| `unless_changed`    | string[]           | —       | Veto when any filtered changed path matches a configured scope name or selector-dialect glob. Supports registered path references.                                                                                     |
