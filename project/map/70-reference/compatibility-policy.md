...
Input enumerations are append-only. The schema comparator keeps only removals under `config-input`. The manifest comparator treats tool input enumerations and the `choices` of a positional or flag the same way. The CLI manifest records those choices from the enum type a command registers, for a positional and for a flag with one enum-typed value, so the rule has real input as soon as a command declares one.
