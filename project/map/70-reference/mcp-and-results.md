...
An unlanded successful `setup done` carries Proof, canonical completion inventory, and landing state without reactivation or improvement advice. After successful `setup accept`, registry-derived `data.reactivation` carries each provider's exact check, local recovery, and CLI fallback; `data.optional_improvement` is explicitly conditional on activation verification. An in-place completion already on the trunk projects the same ordered activation contract.
