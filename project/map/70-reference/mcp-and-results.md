...
The comparison permits the table's additions, reordered contract unions, and the first MCP exposure of an existing CLI contract. A new optional config key must still accept every value admitted for that name by the trunk object's `additionalProperties` schema. Tuple schemas compare `prefixItems` by position. A result-role aggregate may widen only when its top-level branch reachability count is 1, including through a constrained `$ref` or a same-instance applicator such as `allOf` or `dependentSchemas`. An ambiguous aggregate and a nested union stay closed. The comparison rejects policy drift, arbitrary metadata that tries to authorize a union change, removals, and changes to existing types, required result fields, or validation ([ADR 0208](../_adr/0208-public-contracts-version-by-schema-major.md)).
