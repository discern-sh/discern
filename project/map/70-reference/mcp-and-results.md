...
For an artifact already registered on the configured trunk, the gate keeps its repository path and public identity enrolled. The artifact records its compatibility policy, and same-major comparisons use the policy from the trunk artifact. A breaking major adds a publication, artifact, and route while retaining the earlier major. The comparison permits the table's additions, optional config keys, reordered contract unions, and the first MCP exposure of an existing CLI contract in its top-level role aggregate. A nested union cannot gain alternatives because it contains the same role references. The comparison rejects policy drift, arbitrary metadata that tries to authorize a union change, removals, and changes to existing types, required result fields, or validation ([ADR 0208](../_adr/0208-public-contracts-version-by-schema-major.md)).
