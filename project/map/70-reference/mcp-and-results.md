...
| Landing receipt note         | <https://discern.sh/schema/v1/discern-receipt-note.schema.json> | [`schema/discern-receipt-note.schema.json`](../../../schema/discern-receipt-note.schema.json) | The receipt envelope acceptance attaches to a landed commit, using the Dead Simple Signing Envelope (DSSE) field and payload boundary. | Same-major releases may add only optional fields, new contracts, and error slugs. |
