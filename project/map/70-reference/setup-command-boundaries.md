...
For truncated results, consume state, diagnostics, location, next action, recovery, Proof, and relay from the structured or retrievable view. A repeated `setup done` on the unchanged clean marker-bearing commit is the supported read-only retrieval path: `data.completion` is `replayed`, `effects_performed` and `gate_ran` are false, and the same Proof, inventory, and landing facts return. Missing or stale Proof validates that existing commit through one non-forced path. Dirty state refuses without changing the marker or existing evidence ([ADR 0351](../_adr/0351-setup-completion-replays-proof-and-rolls-back-only-owned-tips.md)).
