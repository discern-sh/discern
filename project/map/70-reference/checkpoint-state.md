...
`discern checkpoints` (CLI, `--json`, `--markdown`, and the MCP tool `discern_checkpoints`) reports the governing policy with each checkpoint's criterion and trigger summary, every episode's state, a structural preview of what the current change would fire, recorded episodes outside the governing policy, and fail-open advisories. `discern prepare` and `discern status` serve each coming `stop` criterion early through the advisory channel, and `discern done --dry-run` previews the same projection. Every read surface is effect-free: it runs no configured `when` command (an undecided condition reports as pending) and writes no episode. Command details live in the [CLI reference](cli-reference.md#discern-checkpoints); the result contract in [MCP tools & results](mcp-and-results.md).
