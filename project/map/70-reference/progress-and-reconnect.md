...
A terminal shows counts as a transient status line and pins failures and waits above the live tail. A `--json` or `--markdown` run stays quiet because the result envelope is its entire output; the same facts still reach the journal and any MCP progress token.
