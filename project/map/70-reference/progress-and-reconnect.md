...
Progress is a stream of typed facts ([`src/engine/completion/events.ts`](../../../src/engine/completion/events.ts)): the current phase, a composed sentence, producer-reported counts, an owner-must-act flag, and one fact per failure the moment it is established. Producer facts include the engine-observed running, passed, failed, or cancelled state; the active work labels; reported counts; and the producer's elapsed time. A complete unit count never supplies a verdict. Records without a producer state report the last known facts without claiming that the producer is running. The sentences are composed once ([`progress_prose.ts`](../../../src/engine/completion/progress_prose.ts)) and read identically on a live terminal frame, in static human output, in Model Context Protocol (MCP) `notifications/progress` messages, and in the recorded journal — no surface derives its own account, completion fraction, or time estimate. Unequal units cannot compose into one completion fraction, and an unknown total stays unknown in the words themselves.

A live terminal frame shows counts, waits, and environment steps in its one transient line and pins only failures and owner decisions, because every pinned line adds to the frame's height; static human output writes each durable sentence once and rations counts to a cadence, and the journal keeps everything. Both human modes omit the startup recovery-handle announcement; MCP still receives it, and `discern progress` discovers the latest operation without a handle. Every fact has exactly one presenter ([`progress_presenter.ts`](../../../src/engine/gate/progress_presenter.ts)): a producer's own counts and failures belong to the run executing that producer, and queue, environment, pending, and operation facts belong to the outermost operation — so when `accept` validates an effort, the acceptance narrates the coordination, its nested validation narrates the producers, and no sentence prints twice. A `--json` or `--markdown` run stays quiet because the result envelope is its entire output; the same facts still reach the journal and any MCP progress token.
...
With no handle, `discern progress` reads the most recently started operation. The result leads with the operation named by its branch and what happened to it, then the recorded facts. Each producer appears once with its latest state and counts. Markdown includes active work, transcript paths, and failures without a positional list cap. A retained result renders through the original verb's presenter, preserving its diagnostics, recovery, Proof, and wait continuation. Reading it runs no work. `data.record_path` names the complete journal for timing details and evidence beyond a presentation bound; an oversized result also names its separate file. Full producer output stays with the run's own artifacts: job transcripts under the 24-hour temp retention ([temp files & retention](temp-files-and-retention.md)) and durable attempt artifacts under `discern/completion/artifacts/`.
...
Completion attempts link their renewable publication claim to this handle when the journal is available. A later completion invocation can cancel an unfinished claim immediately when the journal establishes that its executor is gone. A current or uncertain executor remains fenced until its 60-second lease misses renewal. The journal accelerates recovery; the lease remains the authority, so process-identifier reuse cannot prolong a dead claim. A still-current claim returns one pending cause with the handle and directs the caller to read that operation rather than starting another gate ([ADR 0406](../_adr/0406-completion-claims-use-renewable-bounded-leases.md)).
...
Slot acquisition and queue waiting have separate timing categories. `capacity-acquisition` records routine admission, including immediate acquisitions. `capacity-wait` starts only when the slot allocator reports that no slot is available and ends on acquisition or interruption. Markdown summarizes queue waits of at least 100 ms with their duration and explains that test-run capacity is shared across the repository's worktrees. Shorter waits and routine acquisition timings remain in the complete journal.
