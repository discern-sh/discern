...
`done` captures after fix/build and before check/test; `test` captures before its group. An earlier block records `boundary/not-reached`. Complete state covers HEAD, semantic index entries and visibility flags, tracked checkout state including sparse or assume-unchanged entries, untracked entries that Git does not ignore, and recursively initialized clean submodule commits. Dirty, uninitialized, unreadable, unknown, or over-budget state has no combined digest.

The HMAC key stays in a regular `0600` Git-common file at `.git/discern/validation-hmac-key`; unsafe existing artifacts make evidence incomplete. Repository capture is capped at 20,000 paths, 64 MiB, and 5 seconds; execution identity at 1,000 jobs and 1 MiB. The Logbook stores no path manifest, content, command, config value, environment value, or plain content hash. Capture failure cannot change the command verdict. Ignored files, external services, clocks, random seeds, runtime environment, and concurrent external processes are outside the snapshot. Older events omit `validation` and remain readable ([ADR 0273](../_adr/0273-validation-comparisons-require-complete-keyed-semantic-evidence.md)).
