...
Provider-local sits outside these buckets: the coding agent owns it, and discern only keeps it out of Git. Untracked provider files have no discern entry.

## Provenance classes

Shared and Generated artifacts declare:

- **Context-loaded:** no marker. Agent files and skills load whole, charging tokens repeatedly; providers render comments differently. Guidance and drift checks enforce it.
...
`/**` covers a directory. Provider-local stays visible so ignored agent files are not mistaken for generated output.
