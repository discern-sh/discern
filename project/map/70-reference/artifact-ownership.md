...
Project lines outside `.gitattributes`' discern markers remain unchanged. `setup`, `refresh`, and `upgrade` rebuild the block from `discern.toml`, the source-path registry, and the active agent registry. A rebuild replaces hand edits inside the block, but it never edits project rules elsewhere in the root file, nested `.gitattributes` files, or Git's `info/attributes` file ([ADR 0093](../_adr/0093-upgrade-reconciles-gitignore-block.md), [ADR 0259](../_adr/0259-generated-groups-opt-in-to-review-metadata.md)).

The managed block requests `merge=discern-generated` for every tracked output owned by a declared [Generated artifact](../00-orientation/glossary.md#generated-artifact) and every live Agent-file output from the compiler registry. Git still applies its complete precedence rules. `discern doctor` therefore passes each canonical tracked path to `git check-attr --stdin -z merge`; a later root rule, a nested rule, or `info/attributes` can make the effective value unsafe even while the managed block is current. Doctor reports the path and value without rewriting the project rule. Set `linguist_generated = true` inside one `[generated.<name>]` table to mark only that group's paths as generated for GitHub. GitHub then hides them in diffs by default and excludes them from language statistics. The default is false ([ADR 0247](../_adr/0247-generated-artifacts-regenerate-never-merge.md)).
