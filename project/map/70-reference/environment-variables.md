...
Controls for inspecting discern's failure reporting.

| Variable              | What it does                                                                                           |
| --------------------- | ------------------------------------------------------------------------------------------------------ |
| `DISCERN_CRASH_PROBE` | Forces each normal CLI verb or MCP tool call to throw an internal error when set to a non-empty value. |
