...
That runs `deno fmt` (fix), then `deno task codegen` and `deno task site:build` (build), then `deno lint`, `deno check`, and the prose check in parallel with `deno task test` and the `deno task dev --version` smoke test. The release binary build remains outside the gate; the build stage regenerates the checked-in schemas, types, and reference pages and produces the ignored site output the site tests read. A clean checkout should pass; if it does not, `discern doctor` and the [gate gotchas](done-gate-gotchas.md) explain what to fix.
