...
### A Markdown edit fails the diagram-geometry sweep

**Symptom.** The test stage fails in `tests/diagram_geometry_test.ts` ("every fenced box-drawing diagram in tracked Markdown is geometrically sound"), listing `file:line:column` findings such as `"│" connects above but finds only space`.

**Cause.** A fenced code block containing box-drawing characters was edited without re-counting columns. Any fence holding a corner or junction glyph is a diagram: every vertical, corner, and junction must sit in the same code-point column as the glyph it joins on the neighbouring row, and every arrowhead must touch its shaft. No formatter repairs this — `deno fmt` skips fence bodies and `discern tidy` keeps them byte-for-byte — so alignment is part of the edit.

**Fix.** Open each named location and pad with spaces until the columns meet: interior rows as wide as their borders, junctions directly above their hangers. Label text may anchor a line's end (a `│` may hang from a word); space may not. The full rules and tolerances are documented in [`tests/diagram_geometry.ts`](../../../tests/diagram_geometry.ts).
