...
Deno recognizes `deno-lint-ignore` for one line and `deno-lint-ignore-file` for a file. Both forms are migration debt in this repo. Fix the named rule instead of adding either directive. If a rule is wrong for the repo, change the shared lint configuration and explain that policy change in review.
