...
| `prose`     | custom | check | `scripts/prose_check.ts`   | Runs Vale over the configured map after blanking frontmatter and excluding the private tree, emitting SARIF so each finding lands as a file/line/rule diagnostic. The style set pairs the authored house style (`.vale/Discern/`) with generated register styles compiled from `scripts/brand/vale.ts`, scoped per map tier in `.vale.ini`. Fix error-severity findings; review warnings with the page-level prose script. |
