...
- **Put commands in `project/scripts/`, implementation in `scripts/`.** The first is `[scripts].dir`, a command namespace: every file there is listed by `discern scripts` and offered by the Desk to run, so each one is executable, starts with a shebang, and carries a `# desc:` line ([ADR 0360](../_adr/0360-the-project-script-namespace-holds-only-commands.md)). A module the config invokes with `deno run`, or that a sibling imports, belongs in `scripts/` — with a shim in the namespace when it also needs a by-hand entry point, the shape [`canary-audit`](../../scripts/canary-audit) has. [`tests/project_scripts_namespace_test.ts`](../../../tests/project_scripts_namespace_test.ts) holds the boundary and names the repair.
