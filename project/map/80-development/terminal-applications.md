...
[`design_system_dependency.ts`](../../../tests/design_system_dependency.ts) records the expected source root, branch and commit. After a package edit, run its focused checks and commit it, then update the consumer's `DESIGN_SYSTEM_REVISION` to that actual commit before the consumer gate. The guard rejects a different branch, missing foundation ancestry, a dirty source tree or an unrecorded tip. Other package overrides remain forbidden. Use `deno install` to update the lock and `deno task codegen` for generated outputs; linked packages receive source-derived license credits. A historical package Proof must never be claimed for a newer revision.

2A and 3A retain the package worktree after their consumer landing. 4A performs the final upstream integration and full gate, coordinates authorized landing and publication, verifies the immutable release, removes the committed link and temporary receipt, and restores registry-origin checks. Its final consumer gate and visual review use the published package. The temporary source dependency is not permission to publish discern or the package, and unrelated web work remains outside this programme's source ownership.
