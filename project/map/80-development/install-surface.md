...
The tracked-refresh planner runs provider transformations in memory and shares expected-state computations for Agent files, `.gitattributes`, and the ADR index. `status`, `done`, and `accept` consume it; live refresh supplies update's merge-time paths ([ADR 0264](../_adr/0264-tracked-refresh-convergence-precedes-landing.md)).
