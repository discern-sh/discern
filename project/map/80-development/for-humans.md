...
- To land an agent's finished branch, run [`discern accept`](../30-worktrees/lifecycle.md#land-the-reviewed-commit) from its worktree. It honors the clean HEAD's gate proof, or runs the gate when no current proof exists, then fast-forwards your trunk to the branch tip, tears the worktree down, and deletes the merged branch — the trunk is the single landing target (ADR 0110). It refuses a dirty tree (commit first) and a main checkout parked off the trunk. To review or build on work that isn't ready to land, leave it as a branch and pull it into a worktree with `discern start --from <ref>` / `discern update --from <ref>` instead.
