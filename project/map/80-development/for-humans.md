...
- Run the Gate at any time with `discern done`, or its repository alias `deno task gate`. Use `discern prepare` for fixers and checks, and `discern doctor` for the install health check. In this repository, the development wrapper runs each command against the current checkout's Engine.
