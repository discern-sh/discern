...
That sync task owns the only download. Gate prose steps resolve the shared cache and refuse a cold or invalid one with the exact provisioning command, so a network or permission failure surfaces at the ensure boundary (where it is cheap to see and retry), never inside a running gate ([`prose_toolchain_test.ts`](../../../tests/prose_toolchain_test.ts) holds both properties).
