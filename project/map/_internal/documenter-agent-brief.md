...
For a documentation assignment in discern itself, follow this project-owned brief. Its scope manifests are assignment material, not gate configuration. Other projects receive the generic [map maintenance guide](../../manual/20-guides/maintain-project-map.md); they do not need this local machinery.
...
10. **Apply the product voice Skill.** Hold its voice behaviors and banned moves. The gate blocks errors and discern-authored findings except the context-sensitive warnings and suggestions registered for editorial review in the [shared selector](../../../scripts/prose_lib.ts). Before handoff, run `discern scripts prose-page <page…>` on every page you wrote or rewrote. Resolve blocking defects and review advisory findings: try a direct explanation before retaining a useful contrast or qualifier. Counted list introductions and stock scope emphasis remain blocking. All findings still feed the prose-density standard; ADRs retain their reduced third-party style set.
