...
10 main areas · 139 detailed entries · 11 statements of benefit · 78 claims about lists with a fixed membership. The top level gives the shortest account, and the deepest level gives the fullest one.
...
- **Health check** — `discern doctor` verifies the installation without changing it: that the settings make sense and match the expected format version; that declared instructions exist in the computer's standard installed-program list (called `PATH`); that recovery history, saved-change identity, signing tools, hidden-file state, separate-copy settings, and project ownership are safe; and that instruction text, how-to guides, working-copy automation, and supporting-service instructions are connected. It also explains, for every instruction, which steps are the project's and which are discern's. _Facts before judgments, and a misconfigured installation names its own fix._
