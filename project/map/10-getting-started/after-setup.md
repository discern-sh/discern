...
Agent files such as `AGENTS.md`, `CLAUDE.md`, and `GEMINI.md` come from discern's built-in guidance plus your authored sources. The files are committed so a bare clone gives its coding agents the same instructions. Edit the source and run `discern refresh`. The Gate rejects a tracked generated copy that has drifted ([ADR 0128](../_adr/0128-enumerated-ownership-tracked-guidance.md)).
