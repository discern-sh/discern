...
Records describe notes, tags identify source, assets establish availability, and deployment updates the site. A failed deploy leaves publication incomplete even when GitHub has the assets. The maintainer release guide owns recovery and operator verification. The [shared site publisher](publishing.md#publish-a-website-update) also accepts a verified website update independently of a binary release. Its product snapshot stays at the greatest published stable version.
