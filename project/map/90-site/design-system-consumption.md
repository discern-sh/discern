...
The public `/` homepage is authored in [`site/page-src/landing.tsx`](../../../site/page-src/landing.tsx) as a typeset essay built from the Editorial and Marketing adapters. It combines an article header with clustered install and manual actions, an article layout, a numbered contents column, an opening drop cap, a wide pull quote, a framed agent-session figure, end notes around the install moment, a placeholder logo cloud, and a grouped site footer. The contents links use the same reduced-motion-aware smooth scrolling as the docs. Every mechanism claim in the prose links to a numbered note, and the runtime test holds each marker to a resolving note with a return link.

Page-owned composition styles live in [`site/page-src/landing.css`](../../../site/page-src/landing.css). Its `.landing-*` selectors compose the page, and one document-level rule supplies the reduced-motion-aware scroll behavior. Component-owned `.discern-*` selectors stay with the package (the consumer-CSS guard below enforces this). [`site/page-src/landing.js`](../../../site/page-src/landing.js) adds copy buttons to elements carrying `data-copy-command`. The command stays selectable text without JavaScript. The static theme toggle in the header is markup only; the shared `/assets/theme.js` wires every `[data-theme-toggle]` control at runtime. [`site/brand.ts`](../../../site/brand.ts) owns the page's title and description.
