...
## Development-only benefit-art specimen

[`site/page-src/specimens.tsx`](../../../site/page-src/specimens.tsx) frames the reusable [`PersistentTraceArtwork`](../../../site/page-src/benefit-persistent-trace.tsx) component as a focused study for the “Knowledge that compounds” benefit. Retained segments form a centered angular spiral whose opposing bounds match and whose final point meets the centroid-placed mark. One accent point follows the route inward. [`site/page-src/benefit-persistent-trace.css`](../../../site/page-src/benefit-persistent-trace.css) owns the artwork and its reduced-motion frame. [`site/page-src/specimens.css`](../../../site/page-src/specimens.css) owns the editorial preview layout without targeting package-owned `.discern-*` selectors. Fixed light and dark token roots place both readings in the same document.

[`site/specimens.ts`](../../../site/specimens.ts) builds the normal static design-system assets and serves the specimen document plus both authored stylesheets directly from source on the worktree's local port:

```sh
deno task site:specimens
```

The command prints the preview address. The sheet is static HTML, SVG, and CSS with the shared theme controller as its only external browser script. It is absent from `PAGES`, returns `404` through the production handler, and carries `noindex` response headers. [`tests/site_specimens_test.ts`](../../../tests/site_specimens_test.ts) guards that development boundary, paired themes, unique document identifiers, absence of a browser framework runtime, and reserved product-name monospace. [`tests/site_benefit_persistent_trace_test.ts`](../../../tests/site_benefit_persistent_trace_test.ts) guards the SVG title and description, retained layers, moving point, and reduced-motion treatment.
