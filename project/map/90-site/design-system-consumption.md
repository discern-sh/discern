...
[`site/page-src/landing.tsx`](../../../site/page-src/landing.tsx) is the homepage route adapter: it owns the metadata boundary, selects one named composition, and declares that composition's shared and page-owned assets. The selected composition lives in [`site/page-src/clarity-first.tsx`](../../../site/page-src/clarity-first.tsx). Its six sections move from the task-level promise through coordination cost, workflow, outcomes, exact-commit evidence, and setup. A different campaign replaces that one module and the composition-specific assets without absorbing the route metadata or shared conversion behavior.
...
Page-owned composition styles remain in [`site/page-src/clarity-first.css`](../../../site/page-src/clarity-first.css). Its `.v3-*` selectors and its styling of the shared prompt anatomy compose the current page while component-owned `.discern-*` selectors stay with the package. [`site/page-src/clarity-first.js`](../../../site/page-src/clarity-first.js) owns only the current task-stage preview. Page browser assets load as deferred ECMAScript modules, so the shared Copy prompt client can import the site scheduler while remaining independent of the optional composition client. [`site/brand.ts`](../../../site/brand.ts) owns the homepage's exact title and description.
...
- runtime-manifest schema 4, Appearance-scope selection, and the explicit blue Accent contract on both production document roots;
...
The package's browser roles default to monochrome. [`SITE_APPEARANCE`](../../../site/appearance.ts) explicitly emits the symmetric Appearance scopes, activates `data-discern-accent` on discern's roots, and supplies blue hue 255 through `--discern-accent-hue`. The production marketing and docs roots share one rendered contract; the Canon Editor uses that contract independently, while nested fixed-theme specimen and art roots reuse its activation and inherit the hue. This keeps the established semantic color, spacing, focus, motion, and background roles in both light and dark modes without conflating browser Appearance with terminal theme selection. The display role prefers Iowan Old Style where the visitor has it, then uses the bundled Crimson Pro as its first portable fallback. Inter serves body and interface roles, and JetBrains Mono serves code. The build copies the bundled font binaries and SIL Open Font License texts from the selected package asset pack into local generated output. The homepage hero's glow interpolates fully opaque accent-and-canvas colors in `oklab`. It uses no filtered transparency or grain layer, so the browser paints the gradient directly instead of compositing it over the canvas. `site/theme.ts` gives every static composition and document page one pre-paint system-preference bootstrap.
