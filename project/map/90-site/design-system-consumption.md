...
[`site/page-src/benefit-art-bifurcation.tsx`](../../../site/page-src/benefit-art-bifurcation.tsx) owns the reusable inline Scalable Vector Graphics (SVG) artwork and its topology registry. One seed doubles through three levels (two, four, then eight paths), and each terminal path ends in a shared small filled-versus-empty triangle cap. [`site/page-src/benefit-art-bifurcation.css`](../../../site/page-src/benefit-art-bifurcation.css) owns its `bifurcation-art` animation and page composition. Each level draws after its parent level, then exchanges the normalized reveal dash for one complete solid stroke during the shared culmination. A terminal's registry endpoint stays on a static translated group while its origin-centred child scales and fades locally. The complete resting tree is the default, and the motion query removes every enrolled animation while retaining that culmination.
