...
[`site/page-src/landing.tsx`](../../../site/page-src/landing.tsx) owns both the public `/` homepage under construction and the complete predecessor preserved at `/old`. The current route opens with the launch hero and nine empty Marketing sections ready for the final composition. The predecessor retains the signed-off ten-part argument and its compact Standard projection while the new page takes shape. The complete delegation, commissioning, Standard, and Proof specimens remain available from [`site/page-src/specimens.tsx`](../../../site/page-src/specimens.tsx).

The current hero opts into the package's `ResonanceGround` through `HeroBlock`'s generic decorative slot. It is the only Artwork component selected for the compositions bundle; its shared Ground foundation arrives through the package dependency graph. The archived hero does not receive it and retains its original site-owned triangle halo.
