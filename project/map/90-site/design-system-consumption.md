...
[`site/page-src/benefit-art-bifurcation.tsx`](../../../site/page-src/benefit-art-bifurcation.tsx) owns the reusable inline Scalable Vector Graphics (SVG) artwork and its topology registry. One seed doubles through three levels (two, four, then eight paths), and each terminal path meets a shared small filled-versus-empty triangle cap at its left edge. [`site/page-src/benefit-art-bifurcation.css`](../../../site/page-src/benefit-art-bifurcation.css) owns its `bifurcation-art` animation and page composition. One registry-bounded front reveals the intrinsically solid tree from left to right, with a linear interior and easing only at the sweep's outer ends. Each smaller cap sits beyond its registry endpoint on a static translated group while its origin-centred child scales and fades locally. The complete resting tree is the default, and the motion query removes every enrolled animation while retaining that culmination.
