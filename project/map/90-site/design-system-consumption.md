...
The public `/` homepage is authored in [`site/page-src/landing.tsx`](../../../site/page-src/landing.tsx) as a typeset essay built from the Editorial adapters. It combines an article header and layout, a numbered contents column, prose with a drop cap, a wide pull quote, a framed agent-session figure, and end notes around the install moment. Every mechanism claim in the prose links to a numbered note, and the runtime test holds each marker to a resolving note with a return link.
