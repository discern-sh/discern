...
The public `/` homepage is authored in [`site/page-src/landing.tsx`](../../../site/page-src/landing.tsx) as a concise product introduction built from the Editorial and Marketing adapters. It combines an article header, compact product facts beside the main actions, native agent integrations, a short account of shared project knowledge and separate worktrees, proof tied to reviewed code, and a restrained long-form tail about one-way quality limits, project memory, and Discern's small local footprint before the grouped site footer.
...
The benefit tail draws its claims from the generated plain-language feature canon and uses the package's `ArticleLayout`, `Prose`, and `PullQuote` adapters. Page-owned margin notes provide the quiet typographic counterpoint; the homepage does not stage terminal windows or proof receipts as decoration.
