...
The macOS jobs sign those binaries with Developer ID, run the smoke against the signed bytes, submit them to Apple's notary service, and verify the online notarization ticket before creating the checksum. The raw executable format cannot carry a stapled ticket, so a quarantined browser download needs network access when Gatekeeper first assesses it ([ADR 0189](../_adr/0189-macos-release-binaries-are-developer-id-signed-and-notarized.md)).
