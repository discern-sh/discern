...
Both callers pass `secrets: inherit`. A called workflow receives only the secrets its caller passes, even in a job that names an environment. Without it, the deploy step reads an empty token. [`tests/workflow_secrets_test.ts`](../../../tests/workflow_secrets_test.ts) holds every reusable workflow call to that rule.
