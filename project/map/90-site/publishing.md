...
Both callers pass `secrets: inherit`. A called workflow receives only the secrets its caller passes, even in a job that names an environment. Without it, the deploy step reads an empty token.
