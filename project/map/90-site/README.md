...
Generated manual references reuse the canonical definitions. Every follow-up link lands in the offline manual, on the page or section that explains the concept to a human reader. [`scripts/manual_codegen.ts`](../../../scripts/manual_codegen.ts) owns these editorial destinations separately from public URL redirects, so a reading choice does not claim historical ownership of a route. A link with no manual destination fails codegen instead of pointing at the repository, and the cross-corpus test in [`site_docs_test.ts`](../../../tests/site_docs_test.ts) rejects any manual page that links map content there. Each destination agrees with the manual's own search: it is the page that claims one of the map page's names, or it at least mentions the map page's title ([`manual_policy_test.ts`](../../../tests/manual_policy_test.ts)).
