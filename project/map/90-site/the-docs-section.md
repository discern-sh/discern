...
The glossary registry supplies first-mention definition cards to rendered prose. On each Markdown page, the first eligible mention of every canonical term becomes the design system's `GlossaryTerm`; headings, code, emphasis, and existing links keep their authored semantics. Longer compound terms take precedence over shorter terms at the same position. Card definitions come from [`scripts/glossary_registry.ts`](../../../scripts/glossary_registry.ts), and their Markdown links pass through the same route renderer as the page. Later mentions stay plain text. The raw Markdown and terminal editions are unchanged.
...
Permalinks sit beside h2–h4 headings. Their labels do not change heading names for assistive technology. A glossary mention is a semantic `dfn` connected to its definition through `aria-details`, so pointer hover and keyboard focus expose the same content without browser JavaScript. The drawer trigger names its action. Theme and copy controls report results. Drawer and search motion respects reduced motion. Without JavaScript, navigation stays in flow and script-only controls remain hidden.
...
[`tests/site_docs_test.ts`](../../../tests/site_docs_test.ts) iterates the discovered site, so every front door and leaf auto-enrols: it must render for a browser, serve pristine Markdown to text clients and via `.md`, appear in the exact CLI/MCP/site/search/llms guidance sequence where applicable, and keep every local link resolvable after rewriting. The same suite walks the directory-derived ADR set, pins history labels and superseded markers, verifies citation-count parity in every related-decision footer, and keeps decision routes absent from search and llms. Its glossary fixture proves longest-match ordering, one card per term, untouched authored inline semantics, rewritten definition links, unique panel relationships, and the design-system class contract. Synthetic projection fixtures prove that a missing section README or a section-less published leaf fails the build. Unpublished tiers are asserted absent by walking the map directory's complement of the derived public allowlist, so a new contributor tier enrols in the 404 guard the day it is created.
