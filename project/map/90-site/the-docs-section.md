...
On a manual page, the first prose mention of each glossary term opens a summary card. [`glossaryMentions`](../../../site/docs.tsx) derives the phrases from the term registry. A term whose plain rendering sets `match: false` reads as ordinary English, so only its multi-word product phrases match, never the everyday word. A multi-word term also matches its plural and hyphenated forms, and the longest term at a position wins, so "open questions" opens the open-question card rather than one for a shorter term inside it.
