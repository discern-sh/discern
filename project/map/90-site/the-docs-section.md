...
`DocsSite.sitemapRoutes` is the canonical Hypertext Markup Language (HTML) route source for the sitemap. It contains the docs landing, every public Guidance page, the decisions index, and every decision record. Decision routes stay out of `site.pages`. Search and llms prepend `DocsSite.landing` to that Guidance-only sequence, while the sitemap adds project history.
...
`Full manual` expands the same Document Object Model (DOM) tree. In the full state, the control becomes `Focused navigation` and collapses back without moving focus. The state is local to the page and lasts only for that view. The site does not persist it or encode it in the web address. `/docs` and project-history routes have no current Guidance page, so they show the full tree with no disclosure control. Section landings use the ordinary section context. Their model-derived page index in the main column remains the complete section overview.
...
The decision index and every record carry a server-rendered `Project history` notice that directs readers to the manual for current Guidance. Both the index and record page mark superseded records. The docs colophon links to this route family, and the main sidebar omits it. Raw `.md` editions retain the original `_adr/README.md` or record bytes.
...
[`tests/site_docs_test.ts`](../../../tests/site_docs_test.ts) iterates the discovered site, so every front door and leaf enrolls automatically. Each must render for a browser, serve pristine Markdown to text clients and through `.md`, appear in the exact command-line interface (CLI), Model Context Protocol (MCP), site, search, and llms Guidance sequence where applicable, and keep every local link resolvable after rewriting. The full navigation must contain every public page once in canonical order. Every guide page must derive its current page, previous neighbor, and next recommendation. The docs and history fallbacks must declare full mode.
