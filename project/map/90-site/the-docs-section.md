...
`DocsSite.sitemapRoutes` is the canonical HTML route source for the sitemap: the docs landing, every public guidance page, the decisions index, and every decision record. Decision routes stay out of `site.pages`, the guidance-only source used by search and llms.
...
[`tests/site_accessibility_test.ts`](../../../tests/site_accessibility_test.ts) runs axe against the docs landing, one automatically selected page from every public section, and representative decision pages, including the search modal, and rejects serious or critical WCAG findings. Executable browser-DOM guards exercise the complete mobile-drawer focus contract and require every generated permalink to remain outside its heading. Focused contract checks cover reduced-motion, no-JavaScript, print, and other client states that the layout-free audit cannot activate. The Markdown renderer's list tests guard valid nested-list semantics found by this audit.
