...
On a manual page, the first prose mention of each glossary term opens a summary card. [`glossaryMentions`](../../../site/docs.tsx) derives the phrases from the term registry. A term whose plain rendering sets `match: false` reads as ordinary English, so only its multi-word product phrases match, never the everyday word.
