...
The left rail has one server-rendered tree: every public section and page, once, in the same canonical order as docs, search, and the pager. [`docsNavigationProjection`](../../../site/docs.ts) adds context to that tree from the current page's position. An ordinary guide initially shows its current section and page, the previous page as nearby context, and the next canonical page as `Recommended`. A boundary neighbor carries its own section into the focused view. This is reading-order guidance, not an invented task relationship; the task index remains the source for outcome-shaped routes across sections.
