...
Release 0.20.0 adds secondary descriptions to choices and group headings, searching across labels and descriptions, a quieter browsing presentation, successful-frame cleanup policies, and a compact acknowledgement. Its public interactive graph also supplies the adaptive Markdown browser, internal-link navigation, mouse input, typed capability refusals, and terminal restoration needed by the integrated reader. Discern's 5A browser consumes the selection and continuation contracts; the composite Markdown browser remains package-owned when discern adopts it.
