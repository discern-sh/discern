...
[`DocumentLayout`](../../../site/ui/layouts/DocumentLayout.tsx) owns the reading chrome: the package Docs header with the drawer toggle, lockup, search opener, and theme control; the package Docs layout holding the rooted Docs nav with its reference foot links, one `main` landmark with package Breadcrumbs, and the package Table of contents in the rail; and the Search palette in its static mode. Its `renderDocumentPage` selects the Docs bundle, `docs.css`, `docs.js`, and the pre-stylesheet bootstrap that stamps the package drawer's first-paint marker and the site's script-only class. Adapters under `site/ui/components/Document*.tsx` feed each package component from the model, [`DocumentHeading`](../../../site/ui/components/DocumentHeading.tsx) renders a model-derived heading with the package permalink, and [`LeafList`](../../../site/ui/components/LeafList.tsx) renders every model-derived page run. Pages are [`ManualPage`](../../../site/ui/pages/ManualPage.tsx), [`ManualCoverPage`](../../../site/ui/pages/ManualCoverPage.tsx), [`DecisionPage`](../../../site/ui/pages/DecisionPage.tsx), and [`DecisionsIndexPage`](../../../site/ui/pages/DecisionsIndexPage.tsx). Site CSS sets the layout's width token and the shell's composition classes; the columns, rails, and drawer are the package's.
