...
The command-line interface (CLI) pins `@discern-sh/design-system@0.12.2` and uses its root, `./cli`, and `./cli/interactive` graphs. Tests match the resolved version to the lock and exclude React and sibling-checkout paths.
...
Generic display measurement, ANSI stripping, truncation, padding, and wrapping delegate to public package functions through [`src/lib/text.ts`](../../../src/lib/text.ts). The local façade retains hanging indents, the explicit long-token overflow policy, content-shaped reports, terminal-size conveniences, sparklines, and meter projection. `TerminalContext` exposes package Token roles, so feature renderers do not import low-level ANSI or theme helpers around the process boundary. `Logger` and `Out` own channels, groups, and effects; they are not palette authorities. Every human frame managed by discern crosses single-line or multi-line safe text before Components or styling. Raw/exported documents, JSON, HTML, MCP, and project child bytes remain outside that edge.

[`src/lib/prompts.ts`](../../../src/lib/prompts.ts) is the sole product prompt choke point. It maps discern's admission, validation, values, and semantic groups into public package prompts; first-class package heading entries keep every populated group visible without selectable sentinels. [`tests/fixtures/interactive_tty_harness.ts`](../../../tests/fixtures/interactive_tty_harness.ts) drives that production path. The later Desk programme owns a separate full-application real-terminal journey rather than treating this generic prompt proof as Desk-loop coverage.

The Git-derived [`terminal_boundary_guard_test.ts`](../../../tests/terminal_boundary_guard_test.ts) outlaws direct presentation dependencies and imports, prompt/painter bypasses, feature-local process policy, duplicate generic text/triangle/palette authorities, raw control literals, and statically provable unsafe Component props. `@cliffy/command` remains the sole direct Cliffy alias/root/import and the parser/help fact source. Its package-owned transitive graph includes `@cliffy/table`; the guard derives that closure from `deno.lock` and rejects any independently rooted Cliffy presentation package.
