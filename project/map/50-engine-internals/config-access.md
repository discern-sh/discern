...
[`TomlEditor`](../../../src/lib/toml_edit.ts): the `discern config set-*`
commands, `preset`, `skills eject`'s `[skills].dir` recording, migrations, and
`upgrade`'s scaffold reconciliation (restore-if-absent, never rewrite a value —
...
down: `tomlNumber` probes candidate literals against `@std/toml` — the parser
that later reads the file — so a JS-numeric spelling the TOML grammar forbids
(`.5`, `007`) is normalized, never written verbatim.
