...
Managed record banners compare their comment text independently of leading indentation. `discern tidy` owns hierarchy whitespace, including the deeper indent that appears when a real named table follows a banner. Reconciliation preserves the config's current indent while replacing stale prose, so `upgrade --check` reports the formatted config as current.
