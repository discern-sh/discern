...
Schema authors name a section-level Boolean as a verb phrase for what discern does when true: `record_logbook`, `export_port`, `track_ignored_drift`, `stream_output`, `fail_fast`, and `report_in_gate`. A Boolean attribute of a named table entry is an adjective describing that entry: `required`, `prunable`, `neutral`, `linguist_generated`, and `bootstrapped`. Checkpoint trigger fields such as `include_generated`, `new_directory`, `binary`, `deletion_dominant`, and `similar_new_file` name the condition they narrow to and retain that shape. An enum key names the role its values select, as `proof_notes_mode` does.
