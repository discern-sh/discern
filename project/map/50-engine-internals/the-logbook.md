...
A crash (an unexpected throw no verb turned into a result) reaches the event the same way: the recording point that catches it reduces it to a `crash` signature holding the error class name plus one trimmed code location, reports it through a one-slot mailbox in `result_capture.ts`, and completion drains it into the event beside the `failed` outcome. The message stays in the saved crash report. A drain parity guard derives the mailbox set from that module's `take*` exports and requires both recording points to drain every one, so a new mailbox cannot leak state between calls on the long-lived MCP server ([ADR 0247](../_adr/0247-crashes-leave-a-report-and-a-distinct-exit-code.md)).
