...
`status` reads the newest 200 parsed events. Per branch, `last_action` is the newest completion. The reader retains every unmatched fresh begin unless a later-started paired invocation completes under an overlapping non-blocking operation boundary. Common boundaries overlap across the repository; checkout boundaries overlap on the same recorded branch. Refusals, observations, another checkout, and earlier-started invocations cannot retire the begin. The newest retained begin becomes `running` for the compact fleet row. The fleet and Logbook lifecycle readers share this liveness predicate ([ADR 0357](../_adr/0357-lock-boundary-evidence-retires-superseded-logbook-starts.md)). Duration priors carry the recent median, P90, and sample count for each verb, preferring the current config epoch. Consumers can select their own view of the full in-flight set without changing the compact fleet row.
