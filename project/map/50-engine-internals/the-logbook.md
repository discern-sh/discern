...
Context (branch, commit, config, toggle) is gathered from invocation, so `accept` reads its branch before removing the worktree. Acceptance also exposes the configured scope names matched by the pinned landing, including conversational and effort-grant landings. The recorder lifts those names into `scopes` for later readers. Completion waits for the concurrent begin append to preserve pair order. Both paths absorb write failures. With unreadable config, nothing is recorded.
