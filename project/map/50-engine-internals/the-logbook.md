...
Context (branch, commit, config, toggle) is gathered from invocation, so `accept` reads its branch before removing the worktree. Acceptance also exposes matched scope names, which the recorder lifts into `scopes`. Completion waits for the concurrent begin append to preserve pair order. Both paths absorb write failures. With unreadable config, nothing is recorded.
