...
Readers group by epoch **equality**, never by position: parallel worktrees interleave their appends, so a trend's comparable series is every run whose setup (epoch, writer, dominant-client version) equals the newest run's, and runs under other setups are counted and named rather than blended or window-breaking. Events recorded on the dedicated one-time-setup branch are set aside before analysis — a half-wired gate is not the practice ([ADR 0224](../_adr/0224-trend-comparability-is-setup-equality.md)).
