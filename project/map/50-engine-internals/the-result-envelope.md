...
Every output enumeration comes from a vocabulary registry in [`result.ts`](../../../src/shared/result.ts) through [`result_vocabulary.ts`](../../../src/shared/result_vocabulary.ts). `RESULT_OPEN_VOCABULARIES` holds the values the engine only carries or displays; `RESULT_DECISION_VOCABULARIES` holds the values engine code branches on when reading. Each entry keys an `x-discern-…` root name to a declaration name and its members, and the helper builds the strict runtime enum from the entry, binding the instance to its key without global metadata so the MCP-advertised output schemas stay plain. The closed set at launch: validation mode, step outcome, diagnostic severity, proposal direction, evidence purpose, requirement kind, checkpoint mode, exception state, landing-authority kind, and the release comparison's status and publication. Everything else is open. Adding a member means editing one array; a new output enumeration must enrol in one registry or the vocabulary guard fails. The durable proof-note reader widens open vocabularies to strings through the same module, so engine code never branches exhaustively on an open value ([compatibility policy](../70-reference/compatibility-policy.md#the-enum-rule)).
