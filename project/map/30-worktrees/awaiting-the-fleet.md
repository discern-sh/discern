...
`--green` also refuses when no checkout holds the branch at call start: a gate receipt is per-worktree state and dies with the checkout, so once a spent train stage is reclaimed (`worktree prune --contained`, [ADR 0225](../_adr/0225-contained-worktree-reclaim-is-offer-only.md)) its receipt can never reappear. The refusal points at the nearest containing branch — the correct await target all along — and at `--landed` for the literal arrival question.
