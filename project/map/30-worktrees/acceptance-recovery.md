...
Cleanup runs last, in the landing and in the retry that recovers it. The worktree, its resources, and its branch go when the branch holds nothing beyond the landed submission and the landing owes no Proof note. A note that fails to record keeps all three and the journal, which is the note's only retry vehicle; the result's first sentence names `discern accept` from that checkout. That retry records the note, retires the journal, and then applies the same cleanup rule, and a retry whose write still fails keeps everything again. A branch with later commits stays, with `discern done` then `discern accept` as the route; a removal that cannot complete stays, with `discern worktree prune` as the route. A note or cleanup failure never runs the landing transaction again.
