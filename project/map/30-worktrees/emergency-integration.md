...
The preview changes no project state. After the owner approves the displayed plan, repeat the emergency action with the same reason, `--confirmed`, its `--approval-token`, each served `--approve-standard` token, and the same preparation receipt when present. A confirmation missing one of those tokens serves the plan again and names what it lacks. A token expires after 15 minutes. Source, trunk, policy, reason, exception, carried-work, and proposal changes invalidate it. An ordinary grant or previous emergency supplies no consent.
