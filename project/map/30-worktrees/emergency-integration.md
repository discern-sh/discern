...
The repair must be committed in its recorded worktree and contain actual trunk. If it is behind, run `discern update`, review and commit the result, then request another plan. It refuses a source containing another recorded unlanded effort. The [carried-work reader](../../../src/engine/emergency/carried_work.ts) collects each other effort's recorded revisions: a registered checkout's head and submitted commit, a parked head, and the tips of parked and task branches. The refusal names each effort and branch with a revision the repair contains and trunk lacks. The repair's own branch and disposable `integration/` copies never count. Checkpoint judgment, protected policy, and source identity remain preconditions.
