...
`discern accept emergency --reason "Restore service"` presents the source, actual trunk, the commits the repair lands beyond it, reason, and failed, not yet run, or stale machine obligations for owner review. A long commit list shows its newest entries, the total, and a `git log` range that lists them all. It issues no passing Proof. Ordinary acceptance still requires complete strict evidence.

The repair must be committed in its recorded worktree and contain actual trunk. If it is behind, run `discern update`, review and commit the result, then request another plan. When the repair carries another effort's recorded unlanded work, the plan names that effort, and approving the plan lands that work too. The [carried-work reader](../../../src/engine/emergency/carried_work.ts) collects each other effort's recorded revisions: registered checkout heads, submitted commits, parked heads, and parked and task branch tips. The repair's task record adds the commit it started from on another branch. The plan names each effort and branch with the newest revision the repair contains and trunk lacks. The approval token binds that list, and the exception record keeps it. The repair's own branch and disposable `integration/` copies never count. A task started from the repair counts only after it moves from its start point, because until then it holds only the repair's commits. The commit list stays the complete account of what lands; the carried list only attributes commits to recorded efforts. Checkpoint judgment and source identity remain preconditions. Standard limits follow ordinary acceptance's rules. A loosened limit lands only under its current recorded proposal: the plan lists each proposal with its approval token, the confirmation carries the owner's `--approve-standard` token for each, and the exception record keeps the approved proposals. Without a current proposal, or for a redefined or deleted standard, the plan refuses and names the standard.

## Checkpoint preparation

When checkpoint triggers or declarations are pending, run `discern accept emergency --prepare --reason "Restore service"`. This runs only the canonical checkpoint preflight and serves its questions. Repeat with `--met <id>` for each satisfied served question, or `--unmet <id> --why "<rationale>"` for one that isn't, one per call. Unreadable trigger evidence still blocks emergency integration. An unmet answer doesn't: the plan serves it with its question and rationale, the confirmation needs the owner's `--variance <id>` for each, and the ids must match the unmet set exactly, as in ordinary acceptance. The exception record keeps each approved variance. `--dry-run` previews preparation without running triggers or recording conclusions.
...
The preview changes no project state. After the owner approves the displayed plan, repeat the emergency action with the same reason, `--confirmed`, its `--approval-token`, each served `--approve-standard` token and `--variance` id, and the same preparation receipt when present. A confirmation missing one of those decisions serves the plan again and names what it lacks. A token expires after 15 minutes. Source, trunk, policy, reason, exception, carried-work, proposal, and declaration changes invalidate it. An ordinary grant or previous emergency supplies no consent.
