...
Use `accept --target <effort-id> --dry-run` to inspect the resulting landing plan. The preview includes pending decisions and planned composition or producer work. Apply rechecks those facts before each transition. Each earlier effort in the queue needs its own current evidence and authority.
