...
Three more facts lead when they apply. A long operation this checkout started and has not finished leads the result with its verb, its latest recorded sentence, and the `discern progress` handle that reads it back (`data.operation`), and the ordinary start-a-run next steps stay out of the way. A fleet row whose committed source has landed while its checkout stayed carries the same one-sentence checkout outcome the acceptance verdict carries (`landed_checkout`). A trunk tip that landed as an emergency reads as `landed_exception`, with whether its skipped checks are still outstanding, never as a Proof format this build cannot read.
