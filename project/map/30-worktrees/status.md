...
The **Landing queue** lists unlanded efforts in landing order — eligible first, then provisional, held efforts in place — one line each with its readiness and the single reason it waits. An effort whose checks are running reads "checks running now"; an effort a desk grant covers reads as approved; an entry whose checked commit reached the trunk while its branch moved on is told to run `done` for the new work rather than to withdraw. The current worktree's own effort is marked. `accept --dry-run` shows the same list, derived from the same projection, so the two surfaces cannot disagree. `data.queue` carries the rows in the structured result. A source that already sits at the trunk head — the trunk checkout checking itself, or a retained checkout re-validating after its landing — takes no place in the queue; admission settles it as a finished cycle ([`admission.ts`](../../../src/engine/landing_queue/admission.ts)).

Some facts lead the result whenever they apply. A long operation this checkout started and has not finished leads the result with its verb, its latest recorded sentence, and the `discern progress` handle that reads it back (`data.operation`), and the ordinary start-a-run next steps stay out of the way. A fleet row whose committed source has landed while its checkout stayed carries the same one-sentence checkout outcome the acceptance verdict carries (`landed_checkout`). A trunk tip that landed as an emergency reads as `landed_exception`, with whether its skipped checks are still outstanding, never as a Proof format this build cannot read.
