...
A queue row keeps its submitted revision when the branch advances. If the clean branch tip already has complete, valid Proof, recovery names that proven revision and routes to `accept` from its worktree to replace the submission. A new author gate is needed only when current evidence does not support that route. A row whose retained composition awaits a checkpoint decision says so and names the continuation — `discern accept --met`/`--unmet` for a served question, `discern accept --confirmed --variance` for the owner's decision over a declared-unmet conclusion. Status, Desk, and acceptance previews share this [queue derivation](../../../src/engine/worktree/submissions_view.ts).
