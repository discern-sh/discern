...
A proposal-bearing Proof adds a separate owner decision. `accept` serves one token per current Standard/value/reason tuple. It refuses without the complete `--approve-standard` set plus `--confirmed`. Standing and effort grants govern landing coverage. They cannot approve Standard growth. [Landing authority](landing-authority.md#approve-standard-growth) covers the decision and decline path ([ADR 0339](../_adr/0339-standard-growth-proposals-and-shared-measurements.md)).
