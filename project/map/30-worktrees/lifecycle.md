...
Acceptance requires the latest trunk, a clean and unlocked worktree, a tracked-clean main checkout on the trunk, and an empty tracked-refresh plan. Ignored or untracked main data may stay unless landing would overwrite it. An honored `discern done` receipt skips duplicate gate jobs; any later commit invalidates it. Acceptance still checks the current plan, so an older receipt cannot bypass a newer invariant. A failed check leaves the branch and worktree intact ([ADR 0067](../_adr/0067-accept-validates-the-landed-tree.md), [ADR 0264](../_adr/0264-tracked-refresh-convergence-precedes-landing.md)).
