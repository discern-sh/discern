...
Fresh setup has a main-checkout sibling to this boundary. `discern setup accept` lands only `discern-setup` and requires the complete current Proof from `discern setup done`. A fast-forward lands the full commit pinned by that Proof. If trunk moved, the command first merges the sampled trunk commit into the setup branch and requires a new Gate Proof for the merge result. Before trunk moves, it requires an empty tracked-refresh plan and materializes checkout-local agent artifacts; after landing it records the durable Proof note and retires the surviving checkout's local Proof cache. Invalid initial evidence changes no branch or ref ([ADR 0313](../_adr/0313-setup-completion-and-acceptance-bind-one-final-proof.md)).
...
After confirmation, `discern worktree prune` removes clean merged worktrees, stale registrations, orphan directories, reappeared worktree paths, and resource records only from evidence-backed candidate sets. A worktree candidate needs a plain `discern/worktree-ready` marker in its exact Git administrative entry and a branch formed from the configured prefix plus the id encoded by that entry. Merge status, a prefix-shaped name, a checkout-local identity override, or placement under the configured parent cannot create ownership. A prefix branch with no remaining identity evidence stays visible Git context and remains untouched. The setup branch belongs only to setup. Dry-run and apply use the same predicate, and apply rechecks it before removal ([ADR 0315](../_adr/0315-automatic-worktree-cleanup-requires-recorded-ownership-and-verified-absence.md)).
