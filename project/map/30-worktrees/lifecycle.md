...
When the branch holds later commits — an integrated landing compares against the submitted revision, so work added during checking counts — the checkout and branch stay, and the result's first sentence says so and names `discern done` then `discern accept` for them. Uncommitted changes in the checkout keep it and the branch too; the first sentence asks for a commit, then `discern done` and `discern accept`. A failed resource teardown still removes the checkout and branch, and the first sentence names the failed resource and `discern worktree prune`. When cleanup cannot complete for another reason, the landing stands and the first sentence names `discern worktree prune`. A Proof-note failure keeps the checkout, its branch, its resources, and its acceptance journal, and the first sentence names `discern accept` from that checkout: the retry records the note without landing again, then cleans up. With `--target`, the selected submission lands first and the remaining submissions follow the queue's one canonical ordering under their own recorded grants, stopping at the first refusal or failure; `data.landings` reports every attempted landing with the selected member marked, and a stopped walk makes the call false without implying anything landed was undone.
...
Fresh setup uses the same boundary. `discern setup accept` requires current Proof for `discern-setup`; completion replays it read-only or validates the existing clean marker. Acceptance lands the proved commit, or first merges a moved trunk into the setup checkout and proves that result. It checks tracked refresh, materializes local agent artifacts, records the Proof note, then removes the local cache and the setup branch. A note that fails to record keeps both, and running `discern setup accept` again from the setup branch records it. Invalid evidence moves no ref ([ADR 0351](../_adr/0351-setup-completion-replays-proof-and-rolls-back-only-owned-tips.md)).
...
- A Proof-note failure keeps the checkout and its acceptance journal until `discern accept` from that checkout records the note. The retry does not land again.
