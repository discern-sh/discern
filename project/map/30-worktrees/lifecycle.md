...
From the main checkout, `discern worktree drop <id|path>` removes an abandoned worktree and branch. Uncommitted or unlanded work needs human `--force`. A git lock still protects it. If Git cannot read the worktree's status, discern treats its cleanliness as unknown and requires `--force`. The command is CLI-only because agents cannot discard another line of work.
