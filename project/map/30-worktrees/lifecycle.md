...
On first setup, provisioned worktrees with a managed block install `merge.discern-generated.driver=true` in the linked worktree's `config.worktree`. Raw merge keeps the marked side without conflict markers; `done` or `update` regenerates it. Plain clones, CI, and main lack the driver (never global), so conflicts remain; `update` still resolves generated paths. Doctor resolves the effective driver from the checkout it diagnoses, retains Git's scope and origin evidence, and fails a linked worktree whose driver is missing, wrong, or not worktree-local. Repair it with `git config --local extensions.worktreeConfig true` followed by `git config --worktree merge.discern-generated.driver true` ([ADR 0093](../_adr/0093-upgrade-reconciles-gitignore-block.md)).
