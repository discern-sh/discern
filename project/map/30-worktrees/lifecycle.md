...
| Refresh              | Uses the new checkout's engine for the complete shared and checkout-local refresh.  |
...
Only conflicts confined to [generated artifacts](../00-orientation/glossary.md#generated-artifact) auto-resolve; other conflicts abort. After a merge, `update` runs and commits every generator and tracked refresh output, then both ensures. An already-contained source still refreshes and converges. If that changes tracked paths, `update` names and leaves them for review; it creates no no-merge commit ([ADR 0055](../_adr/0055-update-verb.md), [ADR 0059](../_adr/0059-worktree-setup-ensure.md), [ADR 0153](../_adr/0153-repository-owns-shared-checkout-convergence.md), [ADR 0264](../_adr/0264-tracked-refresh-convergence-precedes-landing.md)). Temporary bytes cannot determine output; failures remain visible ([ADR 0247](../_adr/0247-generated-artifacts-regenerate-never-merge.md)).
...
Acceptance requires the latest trunk, a clean and unlocked worktree, a tracked-clean main checkout on the trunk, and an empty tracked-refresh plan. Ignored or untracked main data may stay unless landing would overwrite it. An honored `discern done` receipt skips duplicate gate jobs; any later commit invalidates it. Acceptance still checks the current plan, so an older receipt cannot bypass a newer invariant. A failed check leaves the branch and worktree intact. `discern doctor --verbose` reports that landing-boundary check before the fast-forward, alongside both `done` checkpoints and `update`'s complete refresh/commit-or-report tail ([ADR 0063](../_adr/0063-doctor-execution-model.md), [ADR 0067](../_adr/0067-accept-validates-the-landed-tree.md), [ADR 0264](../_adr/0264-tracked-refresh-convergence-precedes-landing.md)).

On success, discern fast-forwards the trunk and records tracked cleanliness immediately. It writes the receipt note, materializes only local or ignored Agent artifacts, runs `[repository].ensure` and `smoke`, then checks cleanliness again. No tracked refresh writer runs after the fast-forward. It then removes resources, the worktree, and the branch ([ADR 0098](../_adr/0098-accept-refreshes-the-landing-checkout.md), [ADR 0153](../_adr/0153-repository-owns-shared-checkout-convergence.md), [ADR 0215](../_adr/0215-landing-receipts-travel-as-git-notes.md), [ADR 0264](../_adr/0264-tracked-refresh-convergence-precedes-landing.md)). A concurrent landing keeps this worktree for `update → done → accept`.
...
- Receipt-note recording and fetch-configuration reconciliation follow the trunk fast-forward. Their failures report a cause once and cannot fail or undo acceptance; later local-artifact materialization does not retry transport.
