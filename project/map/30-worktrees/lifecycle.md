...
A proposal-bearing Proof adds a separate owner decision. `accept` serves one token per current Standard/value/reason tuple. It refuses without the complete `--approve-standard` set plus `--confirmed`. Standing and effort grants govern landing coverage. They cannot approve a proposed Standard limit. [Landing authority](landing-authority.md#approve-a-proposed-standard-limit) covers the decision and decline path ([ADR 0339](../_adr/0339-proposed-standard-limits-and-shared-measurements.md)).
