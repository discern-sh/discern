...
Fresh setup has a main-checkout sibling to this boundary. `discern setup accept` lands only `discern-setup` and requires the complete current Proof from `discern setup done`. Repeating completion on the unchanged proved tip is read-only; missing or stale Proof validates that existing marker commit without creating another. A fast-forward lands the full commit pinned by Proof. If trunk moved, acceptance first merges the sampled trunk commit into the setup branch and requires new Proof for the merge result. Before trunk moves, it requires an empty tracked-refresh plan and materializes checkout-local agent artifacts; after landing it records the durable Proof note and retires the surviving checkout's local Proof cache. Invalid initial evidence changes no branch or ref ([ADR 0351](../_adr/0351-setup-completion-replays-proof-and-rolls-back-only-owned-tips.md)).
