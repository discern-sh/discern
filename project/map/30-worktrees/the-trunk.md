...
`discern accept` advances the trunk only after conversation consent or a machine-verified standing or effort grant. When the submitted, proven commit already contains the current trunk, acceptance fast-forwards the trunk to it. When the trunk moved after the Proof, acceptance combines the submission with the current trunk in an [integration worktree](../00-orientation/glossary.md#integration-worktree), proves that combined commit, and fast-forwards the trunk to it. A conflict or failed combined check leaves the trunk untouched ([ADR 0391](../_adr/0391-landings-compose-a-moved-trunk-in-an-integration-worktree.md)). Either way, acceptance converges the main checkout and removes the effort's worktree, resources, and branch when the branch holds nothing beyond the landed submission ([ADR 0110](../_adr/0110-the-landing-model.md), [ADR 0194](../_adr/0194-standing-pre-authorization-is-a-recorded-checked-grant.md)). The trunk only ever fast-forwards, so its history is a sequence of accepted, proven commits. Owner decisions that belong to the shared state, such as moving a standard's limit or changing `discern.toml` policy, are made on the trunk by the owner. Agent work lands only through acceptance.
