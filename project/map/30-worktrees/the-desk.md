...
**Join the landing queue** calls `discern accept queue`. It records the reviewed revision and reuses an existing grant. If unattended landing needs permission, the flow reviews queueing and then asks for the separate effort grant. It runs no checks and starts no landing or scheduled task. An active or later walk can pick it up; **Accept** or `discern accept --target <branch>` starts a walk.
