...
This subtree covers discern's scaffolding code under [`src/`](../../../src/):
the [`commands/`](../../../src/commands/) and [`lib/`](../../../src/lib/) that
drive an install. The [Engine](../00-orientation/glossary.md#engine) under
[`src/engine/`](../../../src/engine/) is the run-time half. The **Installer**
writes the bundled seeds from [`templates/`](../../../templates/) into a
project, then refreshes binary-owned files **without clobbering** yours. It
compiles to standalone binaries in `dist/`; installed projects do not need Deno.
