...
The bundled `discern-teach-the-project` Skill provides the procedure for recording a lesson from a conversation in the repository. Ask your agent to “remember this,” “add this to the Guidance,” or “capture this,” and the Skill selects the smallest durable source for that lesson.

discern's built-in Guidance also instructs agents to offer this capture at a natural pause after a user correction, a durable procedure, or an unrecorded decision. The user remains the editor of what the project records. The instruction postpones the offer until active work reaches a natural pause.
...
Record each lesson in one authoritative source. A Guidance rule can link to the ADR that explains it, while the rule and rationale retain separate purposes.
...
4. **Match the destination.** Guidance stays short and imperative. A Skill carries a full playbook. A script executes deterministically. Documentation states current facts. An ADR records a decision's context and trade-off.
5. **Update generated surfaces.** Refresh compiled Guidance, verify a new Skill with the catalog, or run the relevant documentation and script checks. Report what changed and where it lives.
...
- A new authored Skill becomes available after `discern refresh`; Guidance changes also need refresh before agent files become current.
