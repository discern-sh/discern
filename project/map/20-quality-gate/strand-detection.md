...
Ordinary `done` refuses tracked edits, staged changes, and untracked files before any producer runs. Use `prepare` to prepare changes and `test` for focused validation, then commit before completion. An explicit `done --standalone` runs transient diagnostics on a dirty tree and reports strands without earning a Proof.
