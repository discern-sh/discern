...
Use `discern prepare` while you work, and once more before your final commit, so the mutating stages have nothing left to rewrite. It runs the fix jobs, regenerates every declared `generated:<name>` job, then runs the checks. Its package-backed Workflow tracks aggregate progress and every job from `pending` through `running` to its outcome. It injects spinner phase and elapsed time into a pure Gate view; that view never observes the process or decides a verdict. `--plain` and CI make the same Workflow static. Streams and pipes keep their transcript. A successful result names the omitted build jobs and tests, including an empty fix or check configuration.
...
Human Gate views live in [`presentation.ts`](../../../src/engine/gate/presentation.ts). That seam maps typed Gate facts into released design-system CLI Components: Procedure and PrerequisiteList for plans, Meter and Command for work, StandardMeter for Standards, Diagnostic, RawOutput, and RetryNotice for findings, ResultSummary for outcomes, and Receipt for Proof evidence. Collection, scheduling, result envelopes, raw child bytes, and Proof authority stay in discern. Terminal capabilities and safe text arrive from the shared terminal context rather than being re-observed in Gate files.
