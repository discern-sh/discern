...
The gate is the repository's definition of done. It first checks that the branch contains the latest trunk, the branch did not weaken a standard, discern's generated artifacts are current, and the map and guidance hold together — dead links and anchors, stale fenced `discern` examples, unreadable metadata blocks, published pages linking into the internal trees, and citations of skills that do not exist all fail here ([ADR 0199](../_adr/0199-the-gate-ships-the-map-integrity-preflight.md)). It then runs the declared jobs by stage: fix serially, build in parallel, check and test with standard measurements, then gates for the scopes that changed. A stage that changes a committed-clean tracked file also fails the run, so generated output cannot hide behind a green result ([ADR 0148](../_adr/0148-strand-detection-covers-every-gate-stage.md)).
