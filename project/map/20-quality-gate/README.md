...
Use `discern prepare` while you work. Its buffered terminal table tracks every fix and check job from `pending` through `running` to its outcome before the final tail. `--plain` and CI make the table static. Streams and pipes keep their transcript. Success says build and test did not run and reports an empty fix/check configuration.
