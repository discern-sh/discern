...
- **The line**: one sentence naming the branch, validated commit, diffstat, Standards state, any Standard limit proposals, and the page command. A single proposal appears with its Standard and limits; several appear as a count. JSON and MCP carry it as `data.proof.line`; `accept` derives `data.proof_line` from it: each segment that awaited an owner decision is rewritten in place to its resolved state — proposal approved, variance authorized — and the recorded consent source closes the line. A landed line never states a pending decision beside its own resolution. Agents quote that line verbatim after their account.
...
The Proof line carries the open proposal as a present-state fact — its Standard, trunk limit, and proposed limit, awaiting the owner's exact approval. The page presents the proposal before routine Standard results. Compact JSON, Markdown, Model Context Protocol results, status, and proof notes retain the structured proposal. A green proposal-bearing Proof establishes Gate success for that committed tree. It does not grant landing authority or approve the proposal.
