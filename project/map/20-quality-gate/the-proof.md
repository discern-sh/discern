...
- **The line**: one sentence naming the branch, validated commit, diffstat, Standards state, any proposed Standard limits, and the page command. JSON and MCP carry it as `data.proof.line`; `accept` derives `data.proof_line` from it and appends the recorded consent source and approved proposal count. Agents quote that line verbatim after their account.
- **The page**: proposed Standard limits awaiting a decision, routine Standards, declared jobs and scope gates, then the diff command. It stays in the worktree marker and landed Proof note. Terminal `status --verbose` prints a valid page. Git owns commit and per-file lists; `Inspect:` names the command.
...
After the trunk fast-forward, normal and setup acceptance write separate result and presentation blocks to a DSSE-compatible note under `refs/notes/discern`. Normal acceptance evidence includes each owner-approved Standard limit. The local unsigned record is on by default and fail-open. Transport is opt-in. [Proof notes](proof-notes.md) covers inspection, publication, and recovery.
