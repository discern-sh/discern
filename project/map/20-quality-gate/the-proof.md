...
- **The page**: Standards, declared jobs and scope gates, then the diff command. It stays in the worktree marker and landed Proof note. Human `status --verbose` prints a valid page. Git owns commit and per-file lists; `Inspect:` names the command.

The complete in-process Proof owns both renderings. Agent wire results use a compact projection with branch, trunk, validated commit, diff counts, and line. They omit the page, which can otherwise appear several times in one status fleet. `discern <verb> --markdown` selects an authored result presentation; it does not substitute the full Proof page for that presentation.
...
| `discern status` | Reports whether the marker still matches the clean current `HEAD`. Agent results return Proof status plus compact facts; human `--verbose` retrieves the page. Status also reads a landed trunk-tip Proof from the local or fetched notes ref as `data.landed_proof`. |
