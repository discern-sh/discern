...
A command may therefore emit both a scalar health measure and counts that hold richer sets. The repository's coverage command emits aggregate coverage, module-floor failure count, and reviewed-exception count from one instrumented test run. The zero failure-count ceiling is the enforceable scalar; the command output names the member paths and recovery, preserving the set-valued diagnosis. The exception-count ceiling ratchets debt independently. Do not substitute the lowest module percentage: it loses which modules failed and makes legacy exceptions indistinguishable from compliance ([ADR 0342](../_adr/0342-git-elects-module-coverage-membership.md)).
