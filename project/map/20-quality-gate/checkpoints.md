...
The gate still runs and can go green, and the work then waits for a decision. A current declared-unmet conclusion makes `discern accept` refuse with `error: "awaiting_variance"`, serving the question, the matched evidence, and the rationale. The owner authorizes each named [variance](../00-orientation/glossary.md#variance) in the current conversation with `discern accept --confirmed --variance <id>`; the id set must equal the declared-unmet set. Recorded standing and effort grants never authorize a variance. Each authorization binds to the exact declaration and the landed commit, and changes no future policy. For an integrated landing the interlock resolves over the combined result's own declarations in the retained integration worktree — a question the composition fires is served through acceptance with the composition's receipt and answered with `discern accept --met`/`--unmet --composition-receipt <receipt>` about that exact combined subject, never inherited from a different one ([ADR 0395](../_adr/0395-integration-judgments-continue-the-retained-landing.md)).

## The shipped set

A fresh install's config activates 10 built-ins. Four `stop` members guard knowledge surfaces that steer future sessions: `map-focus` (a new explanation or substantial rewrite must reduce future reading), `instruction-economy` (the configured instruction sources must earn their always-loaded cost), `skills-playbook` (a skill is an executable playbook), and `gotchas-playbook` (failure memory stays symptom, cause, and proven recovery; it remains inactive until `[project].gotchas_doc` names a document).
...
`map-focus` selects new current map pages and changes of at least 80 lines across existing current pages. Small corrections stay quiet. `map-drift` selects changed sources linked from current explanations and returns those pages as related evidence; changing a map page does not cancel the review. Links in the previous version of an edited or deleted page still count. Without specific links, five changed authored files prompt a broader advisory. Missing map evidence also prompts review. Generated pages, ADR history and private notes stay outside the current account. Configured trigger fields can narrow these built-in selections further.

The source-link reader is shared with map freshness. Neither trigger judges truth: the agent checks the affected explanation, updates it when needed, and can leave an accurate page unchanged.
