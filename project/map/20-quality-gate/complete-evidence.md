...
An unfinished attempt holds a [60-second renewable publication claim](../_adr/0406-completion-claims-use-renewable-bounded-leases.md), independent of project timeout budgets. [`attempt_lifecycle.ts`](../../../src/engine/completion/attempt_lifecycle.ts) renews a live coordinator every 20 seconds and stops renewal before settlement. Before reserving another attempt, completion and committed standards measurement cancel an exact journal owner that is gone or a claim whose renewal expired. Cancellation lands through compare-and-swap before the replacement receives a new fencing token, so the retired executor cannot publish. If the existing claim is still current, the result reports one pending cause and its progress handle rather than assigning a failed gate stage.
