...
| Behavior   | Red `done` streaks, repeated refusals with one slug, `done`-only iteration with no `prepare`, hint follow-through by declared family, dirty-tree churn, edits on the trunk, recurring `--force`, recurring `done --confirmed` reruns, pre-authorized landing audits, grant suggestions, missed doc lookups, worktrees started but never green, out-of-protocol orderings, recurring drivers the identity catalogue can't name, a returning agent whose native integration isn't configured, red streaks split by driver cohort, and a guidance-parity read of gaps one cohort keeps hitting. |
...
`pre-authorized-landings` waits for 8 consent-recorded landings, 3 granted. It reports count, share, source and standing-scope splits, runs of 4, and half-to-half shifts of 30 points. Older accepts lacking consent evidence are excluded.

`grant-suggestion` names the scope and `[acceptance].pre_authorized` after the newest 12 accepts land conversationally in one scope. A refusal, another source, missing scope, or second scope breaks the run. The owner edits the trunk's `discern.toml`; discern never writes grants.
