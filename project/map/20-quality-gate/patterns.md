...
| Behavior   | Red `done` streaks, repeated refusals with one slug, `done`-only iteration with no `prepare`, hint follow-through by declared family, dirty-tree churn, edits on the trunk, recurring `--force`, recurring `done --confirmed` reruns, pre-authorized landing rates and scope splits, conversational single-scope runs that may support a standing grant, missed doc lookups, worktrees started but never green, out-of-protocol orderings, recurring drivers the identity catalogue can't name, a returning agent whose native integration isn't configured, red streaks split by driver cohort, and a guidance-parity read of gaps one cohort keeps hitting. |
...
`pre-authorized-landings` waits for 8 landings with recorded consent evidence and at least 3 that used a standing or effort grant. Its summary gives the count and share, splits standing from effort, and names a run of 4 or a 30-point change between the earlier and later halves. One row per standing scope shows how often that grant covered a landing. Accepts recorded before consent evidence existed stay outside the denominator.

`grant-suggestion` examines the newest `accept` attempts. It suggests considering a standing grant after 12 consecutive attempts succeeded with conversation consent and each changed the same single scope. A refusal, another consent source, a second scope, or missing scope evidence breaks the run. The finding names the scope and `[acceptance].pre_authorized`. The owner types that grant into the trunk's `discern.toml`. discern never writes it.
