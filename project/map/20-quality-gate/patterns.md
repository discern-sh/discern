...
`checkpoint-dead`, `checkpoint-noisy`, and `checkpoint-varied` read the shared economics tallies and advise on configuration fit; no finding evaluates an agent. The first two count gate-run efforts since the newest recorded `[checkpoints]` config change, so a revised definition earns fresh evidence: after 8, a checkpoint that never fired reads as mis-scoped, and one served on at least 80% reads as taxing every change. `checkpoint-varied` speaks after 3 landed efforts where the checkpoint fired, at least half under an owner-authorized variance. Findings give counts beside denominators and recommend reviewing the trigger, criterion, or mode.
