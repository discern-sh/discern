...
The recorder stores raw driver signals; scoring them is reader logic here, computed fresh on every read — so when the scoring improves, all accumulated history benefits without migration. CI runs and `--dry-run` previews reach no detector, and runs that look interactively driven never count toward agent-behaviour findings — your own exploratory poking is not agent pathology ([ADR 0162](../_adr/0162-logbook-day-one-vocabulary.md)). Events recorded on the dedicated one-time-setup branch are set aside before analysis and counted in the report header: gate runs made while the gate was being wired describe a project being stood up, not the practice, and would otherwise read as trends — a sub-second "gate" accelerating to its real duration on day one ([ADR 0224](../_adr/0224-trend-comparability-is-setup-equality.md)).
...
Trend detectors compare only runs sharing one setup: one config epoch, one discern release, and one version of the dominant MCP client — the agent's own releases move the practice too. Comparability is equality on those three, never position in the stream ([ADR 0224](../_adr/0224-trend-comparability-is-setup-equality.md)): parallel worktrees carry their own configs and append to one shared logbook, so one setup's runs are routinely interleaved with runs from other setups, and they still form one series. When runs under other setups exist beside a too-short series, the report counts them and names what differs (the config section that moved, or the version pair) instead of blending incomparable runs or staying silent ([ADR 0190](../_adr/0190-cohort-findings-lift-the-provider-comparison-deferral.md)). A corpus with no client declarations has no client dimension, and its trends compare as before. Where rotation has removed raw months, the prune digests extend coarse series, marked as coarse.
