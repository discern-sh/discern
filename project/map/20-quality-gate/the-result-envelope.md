...
([ADR 0117](../_adr/0117-temp-output-artifacts-are-reaped-by-age.md)).
...
- **Run time** —
  ...
  the real cores and asserts each one's `serializeResult` output validates
  against its schema. An enrollment guard in the same suite iterates the
  contract registry: every published contract must either have such a
  faithfulness test or sit in an explicit debt list (never both, never neither),
  and a contract advertised as an MCP tool's `outputSchema` may never be debt —
  so a new verb can't publish a contract nothing exercises.
