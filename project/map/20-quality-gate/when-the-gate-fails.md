...
| The maintained ADR index is out of date     | Run `discern refresh` and commit the rewritten README. If the diagnostic names a record file, fix that record's first heading instead — it must carry the number and a title. |
