...
| A Skill's frontmatter is invalid                     | Edit the named `SKILL.md` until its block is YAML every parser reads the same way: `name` and `description` on single lines, quoted when a value contains `:`.         |
