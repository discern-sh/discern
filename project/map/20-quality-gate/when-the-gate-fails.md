...
| discern's write-access preflight was denied  | Authorize the exact path named by the diagnostic for the retry invocation, then run its exact `reproduce_cmd`. The probe stopped before project jobs ran. A later successful probe is only point-in-time evidence for that invocation; discern does not grant or retain provider authority.                                                                                             |
...
| `done` refused an unchanged-tree rerun       | Fix or change the failed state before running `done` again. For an explicit same-state or environment-only retry, run `discern done --rerun`; discern records the probe. Existing `done --confirmed` scripts remain compatible, but `--rerun` is the Gate-specific spelling. A valid exact green Proof would have returned successfully without running jobs.                           |
