...
- [ ] **The scriptorium's structural rung (rung 3 of the commissioning proposal).** The studio (ADR 0288) ships reading, in-place prose write-back, and live register lint. Deliberately deferred until real launch-copy mileage says which operations matter: typed pickers for list fields (`drawsOn`, `claims`, `surfaces`, `hints` — the rail shows them read-only today), scaffolded add/retire/reorder forms with absence records, a stage-and-commit composer, and the "brief an agent" escape hatch for campaign-sized changes. Evidence: `scripts/scriptorium/fields.ts` (list fields already classified with picker sources); `project/map/80-development/the-scriptorium.md` (§Limits).
