...
- [ ] **Let the CLI presenter bind foundation motif and box renderers.** The presenter accepts Component renderers whose props carry a theme, but the package's triangle motifs and `renderBox` use separate signatures. Discern therefore passes its presenter's bound capabilities to those foundation APIs directly while every Component renders through `present`. Add presenter-owned motif/box entry points upstream, then remove the remaining direct renderer calls. Evidence: `src/engine/gate/presentation.ts`; `src/engine/status/tty.ts`; `src/engine/improve/improve.ts`; `src/engine/logbook/patterns.ts`; `src/commands/setup_welcome.ts`.
