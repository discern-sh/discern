...
- [x] **Align generator-owned public vocabulary with the glossary.** The CLI and config registries now say “agent files” and “guidance source”; codegen carries those names into the generated references. Evidence: `src/engine/dispatch.ts` (update and refresh descriptions); `src/main.ts` (upgrade description); `src/shared/config_schema.ts` (`guidance.sources` description); `src/shared/paths_registry.ts` (guidance-source description); `project/map/70-reference/cli-reference.md`; `project/map/70-reference/config-reference.md`.
