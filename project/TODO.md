...
[`project/map/`](map/README.md) describes what _currently exists_; this file
...
- This is a backlog, not documentation — modal verbs ("should", "could") are
  fine here, unlike in `project/map/`. Don't write that something was "finished
  this round" — that's meaningless to a future reader ("which round?"). Every
  item must be pickup-able at any later time with no session-specific context
  required.
...
- [ ] **Version the public JSON result contract before launch.** The generated
      ...
      `project/map/20-quality-gate/the-result-envelope.md`.
...
- [ ] **Revisit generated-artifact user-content preservation only with a
      ...
      `project/map/_adr/_superseded/0091-rescue-generated-content-before-overwrite.md`;
...
- [ ] **Scaffold the CI gate workflow once releases are public.** The docs now
      ...
      writes the workflow for users. Evidence:
...
- [ ] **Site launch blocker: self-host fonts and precompile Tailwind on the four
      ...
      visible style flash. Before switching any of those routes for launch,
      migrate it to the compiled design-system path proven by
...
- [ ] **Site launch: create the Deno Deploy org + app and point DNS.** One-time
      ...
      `project/map/90-site/publishing.md`.
...
- [ ] **Make "author once → compile everywhere" + per-agent wiring a first-class
      message.** discern compiles one `project/guidance.md` into every vendor's
      agent files and wires each agent's exact guidance file, skills dir, MCP,
      and hooks — most tools just say "supports Claude, Codex, Gemini". Elevate
      this from a buried detail to a headline principle in external docs and
      landing copy.
