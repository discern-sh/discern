...
- [ ] **Owner: confirm the desk menu heights in the real JetBrains terminal, tracing if anything still shrinks.** The crushed-window mechanism (choice requests without a row budget falling to the design system's five-row default) is cured and PTY-guarded, but the reported _progressive_ shrink was never reproduced in-harness and every in-process sizing authority is per-frame stateless — if a second mechanism exists it is terminal-side. One run captures the verdict: `DISCERN_INTERACTION_TRACE=/tmp/discern-interaction.jsonl discern desk` in the full-height JetBrains terminal, then navigate board → task → action menu → Back five times and quit. Every menu should fill the height. If any window still shrinks, keep the JSONL — each line records the viewport at open, every height reading the driver sampled, the derived row budget, per-write line counts, and the outcome — and file it with the terminal name; the numbers distinguish an environment-size fault from a fitting fault. Delete this item if all menus hold. Evidence: `src/lib/terminal_interaction.ts` (`traceInteractionIo`); `tests/interactive_tty_test.ts` (composed-cycles journeys).

- [ ] **Ask the design system for reserved-rows-aware fitting ("fill the available height").** The package fits each interaction frame to the full viewport (`fitInteractionFrame`, `@discern-sh/design-system@0.14.0` `src/cli/interactive/viewport-budget.ts`); a caller that keeps its own header above the interaction can only bound _control_ rows (`visibleCount = rows − reserved`), while the frame's own chrome (heading, box borders, footer) still paints beyond that bound. On lists taller than the budget, the composition's top rows can scroll off by roughly the frame-chrome height. Wanted: a request option that reserves caller rows inside the package's per-frame fitting, so the honest bound is `frame ≤ viewport − reserved` with the same per-frame recovery. Adopt it at `visibleRowBudget` in `src/lib/terminal_interaction.ts` when a release provides it (ADR 0279: generic gaps are fixed upstream, never patched around).
