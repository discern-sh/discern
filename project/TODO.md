...
- [ ] **Build Docs Studio for the public manual.** A local, loopback-only authoring studio launched from a selected worktree through a Project Script: a Library, Editor, and exact-outcome workspace that previews an in-memory draft through the production renderer; plan-and-apply writes confined to the worktree (autosave, create, reorder, rename with redirects, withhold and include) that compare before writing and refuse generated regions; live diagnostics from the project's own validators; then review, update, prepare, commit, `discern done`, and acceptance through the existing cores with truthful Draft, Ready, Landed, Released, and Live states, and no CMS database, telemetry, or agent auto-publish. The 2026-08 briefs assumed the Map was the manual authority, predating the manual split, and were removed on 2026-09-12; specify the studio afresh against `project/manual/` and its kinds and checkpoints after launch. Evidence: `project/map/_adr/0314-separate-public-manual-and-project-map.md`; `src/lib/manual.ts`; `site/docs.ts`.
