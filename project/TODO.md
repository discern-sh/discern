...
- [ ] **Resolve the canonical-set registry's open ownership decisions.** Enrol or explicitly classify the side-restrictions registry and decide whether the maintainer-only canonical-sets meta-registry needs a Glossary term. Evidence: `src/engine/worktree/side_restrictions.ts`; `scripts/canonical_sets.ts`; `project/map/_internal/registry-atlas.md`.
...
- [ ] **Give a fired standard a completion-shaped proposal exit.** Add `discern standards propose <name> --reason "…"` so intrinsic growth can join the exact Gate proof and receive an owner decision during acceptance without creating a standing bypass or editing the trunk limit early. Evidence: `src/engine/gate/standards.ts`; `src/engine/gate/proof.ts`; `project/map/20-quality-gate/standards.md`.
