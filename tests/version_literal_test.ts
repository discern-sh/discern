...
Deno.test("authored TypeScript never hardcodes the package semver", async () => {
  assertEquals(
    ...
    [],
    `import DISCERN_VERSION instead of hardcoding ${DISCERN_VERSION}; sample semvers ` +
...
Deno.test("every version-reporting surface derives from DISCERN_VERSION", async () => {
  await withTempDir(async (dir) => {
    ...
    assertResultDataKey(doctorJson, "discern_version");
    ...
    const surfaces: ReadonlyArray<[string, string | undefined]> = [
      ...
      [
        "scaffolded config provenance banner ({{discern_version}})",
