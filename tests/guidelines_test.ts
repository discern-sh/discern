...
Deno.test("compileGuidelines: banner + built-in + sources; copies built-ins, symlinks authored, prunes", async () => {
  ...
  try {
    ...
    // AGENTS.md is the canonical, tracked agent file: a generated banner, then
    // discern's built-in harness guidance (the base section names the gate
    // command), then the user's own guidance.md appended after it.
    const agentsMd = await Deno.readTextFile(join(tmp, "AGENTS.md"));
    assert(
      agentsMd.startsWith("<!-- GENERATED by"),
    ...
    // CLAUDE.md is NOT a byte-for-byte duplicate: it points at AGENTS.md via Claude
    // Code's `@` import (with a do-not-edit banner), so the two can never drift.
    const claude = await Deno.readTextFile(join(tmp, "CLAUDE.md"));
    assert(
      claude.startsWith("<!-- GENERATED by"),
      "the pointer carries a do-not-edit banner",
    );
    assertStringIncludes(claude, "@AGENTS.md");
    assert(
      !claude.includes("discern finish"),
      "the Claude mirror imports the guidance, it does not inline it",
    );
