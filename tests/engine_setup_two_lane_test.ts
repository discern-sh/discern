 ...
 * data carries a prose `guidance` string must embed that string VERBATIM in its
 ...
 * in the public result registry for a `guidance` field in its data schema, and
 * a set-equality guard forces each member to have a driver here — so a new
 * guidance-carrying verb fails this suite until its human render is proven,
...
/** True when any data variant of the contract's schema carries `guidance`. */
function carriesGuidance(outputSchema: z.ZodType): boolean {
  return dataVariants(outputSchema).some((o) => "guidance" in o.shape);
}
...
/** How to drive one guidance-carrying contract to a run whose data carries a
 * non-empty `guidance` — the same argv is run with and without `--json`. */
...
const DRIVERS: Record<string, TwoLaneDriver> = {
  // The `setup` envelope's guidance carrier is the awaiting-consent refusal: a
...
Deno.test("every guidance-carrying result contract has a two-lane driver (enrolment)", () => {
  const carrying = CLI_JSON_RESULT_CONTRACTS
    .filter((contract) => carriesGuidance(contract.schema))
    .map((contract) => contract.id)
    .sort();
  assertEquals(
    carrying,
    Object.keys(DRIVERS).sort(),
    "a contract whose data carries `guidance` must prove its human render embeds it — add a driver above (or retire one whose schema dropped the field)",
  );
});

Deno.test("the guidance detector catches a fresh-name future sibling and ignores non-carriers", () => {
  // The adversarial future sibling: unrelated verb, unrelated field names, the
  // guidance field buried in one option of a data union — detected purely by
  // shape, with no case-table edit.
  const sibling = z.strictObject({
    ok: z.boolean(),
    verb: z.literal("orbit calibrate"),
    data: z.union([
      z.strictObject({ calibration: z.number(), guidance: z.string() }),
      z.strictObject({ aborted: z.boolean() }),
    ]).optional(),
  });
  assert(carriesGuidance(sibling), "a fresh-name carrier must be detected");
  ...
  assert(!carriesGuidance(nonCarrier), "a non-carrier must not enrol");
});
