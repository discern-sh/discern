...
/** Exec-style CLI boundaries that intentionally write no logbook events. */
const LOGBOOK_UNRECORDED_CLI_VERBS: ReadonlySet<string> = new Set(["queue"]);
...
Deno.test("every non-exec CLI verb routes through the logbook recording wrapper", () => {
  ...
  assertEquals(
    sorted(missing),
    sorted(LOGBOOK_UNRECORDED_CLI_VERBS),
    "CLI verbs whose actions bypass the logbook recording wrapper — wrap each " +
      "action in recordedExit(<verb>, …), or record a reviewed exec-surface exception",
  );
...
Deno.test("every effectful CLI verb auto-enrols in begin recording", () => {
  ...
  assertEquals(
    ...
    sorted(
      [...LOGBOOK_EFFECTFUL_VERBS].filter((verb) =>
        !LOGBOOK_UNRECORDED_CLI_VERBS.has(verb)
      ),
    ),
