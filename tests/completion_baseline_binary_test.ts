...
Deno.test("E16: every representative target measures exactly its freshly built regular output", async () => {
  await withTempDir(async (root) => {
    ...
    for (const target of BUILD_TARGETS) {
      let builds = 0;
      const size = await measureBinarySize(target.triple, root, async (triple, cwd) => {
        builds++;
        assertEquals(triple, target.triple);
        assertEquals(cwd, root);
        await Deno.writeFile(join(root, "dist", target.output), new Uint8Array(137));
      });
...
Deno.test("E16: failed, missing and non-regular builds cannot borrow an earlier binary reading", async () => {
  ...
  for (const outcome of ["failed", "missing", "directory", "symlink"] as const) {
    await withTempDir(async (root) => {
      await Deno.mkdir(join(root, "dist"));
      const output = join(root, "dist", target.output);
      await Deno.writeFile(output, new Uint8Array(137));
      let builds = 0;
      await assertRejects(() => measureBinarySize(target.triple, root, async () => {
        builds++;
  ...
  let builds = 0;
  await assertRejects(() => measureBinarySize("unregistered-target", "/unused", () => {
    builds++;
