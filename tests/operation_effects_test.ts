...
Deno.test("accept queue owns submission publication without landing or Git-write authority", () => {
  const queued = operationEffectPolicy("accept", { action: "queue" });
  ...
  assertEquals(
    operationEffectPolicy("accept", { action: "queue", dryRun: true })
      ?.lock,
    "none",
  );
  assertEquals(
