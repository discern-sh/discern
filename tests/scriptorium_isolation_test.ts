/**
 * The scriptorium's mutation suite must remain hermetic under the repository's
...
Deno.test("the scriptorium mutation suite cannot write into the checkout", async () => {
  const sandbox = await Deno.makeTempDir({
    prefix: "discern-scriptorium-isolation-",
  });
  try {
    const output = await new Deno.Command(Deno.execPath(), {
      args: [
        ...
        "--allow-run",
        "--no-check",
        "tests/scriptorium_patch_test.ts",
      ],
      cwd: REPO_ROOT,
