...
- [0001 — Adopt discern for development with coding agents](0001-adopt-discern.md)

<!-- END GENERATED: current ADR records -->

The list above maintains itself: `discern refresh` rewrites everything between the markers from the record files in this directory, and the quality gate refuses an index that has drifted from them. Add or supersede a record, run `discern refresh`, and commit the rewritten list with the record — never edit between the markers by hand.
