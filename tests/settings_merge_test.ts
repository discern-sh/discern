...
Deno.test("renamed managed hook commands replace legacy and already-duplicated groups exactly once", () => {
  const aliases = [{
    canonical: "discern worktree hook hydrate",
    retired: [
      "discern worktree hydrate",
      "discern worktree init-hydrate",
    ],
  }];
  const incoming = JSON.stringify({
    hooks: {
      WorktreeHydrate: [{
        hooks: [{
          type: "command",
          command: "discern worktree hook hydrate",
          timeout: 60,
        }],
      }],
    },
    ...
    hooks: {
      WorktreeHydrate: [
        { hooks: [{ type: "command", command: "user-hook" }] },
        {
          hooks: [{
            type: "command",
            command: "discern worktree hydrate",
          }],
        },
        {
          hooks: [{
  ...
  const merge = jsonSettingsMergeWithHookCommandAliases(aliases);
  const once = merge(existing, incoming);
  const parsed = JSON.parse(once) as {
    model: string;
    hooks: {
      WorktreeHydrate: Array<{
        hooks: Array<{
          type?: string;
          command: string;
          timeout?: number;
  ...
  assertEquals(parsed.model, "user-model");
  assertEquals(parsed.hooks.WorktreeHydrate, [
    { hooks: [{ type: "command", command: "user-hook" }] },
    {
      hooks: [{
        type: "command",
  ...
  assertEquals(merge(once, incoming), once);
});

Deno.test("renamed hook command convergence covers every supported JSON hook shape", () => {
  const canonical = "discern worktree hook hydrate";
  const retired = "discern worktree hydrate";
  const incoming = JSON.stringify({
    hooks: {
      NestedCommand: [{
      ...
      NestedBash: [{
        hooks: [{ type: "command", bash: canonical }],
      }],
      GroupCommand: [{ command: canonical }],
      GroupBash: [{ type: "command", bash: canonical }],
    },
  });
  const existing = JSON.stringify({
    hooks: {
      ...
      NestedBash: [{
        hooks: [{ type: "command", bash: retired }],
      }],
      GroupCommand: [{ command: retired }],
      GroupBash: [{ type: "command", bash: retired }],
    },
  });
  const merge = jsonSettingsMergeWithHookCommandAliases(
    [{ canonical, retired: [retired] }],
    mergeJsonSettingsDedupingGroups,
  );
  const parsed = JSON.parse(merge(existing, incoming)) as {
    hooks: Record<string, unknown[]>;
  };

  assertEquals(parsed.hooks, JSON.parse(incoming).hooks);
  for (const groups of Object.values(parsed.hooks)) {
    assertEquals(groups.length, 1);
  }
});

Deno.test("renaming a discern command inside a mixed user group preserves the group", () => {
  const canonical = "discern worktree hook hydrate";
  const retired = "discern worktree hydrate";
  const existing = JSON.stringify({
    hooks: {
  ...
  assertEquals(merged.hooks.WorktreeHydrate, [{
    matcher: "user-owned",
    hooks: [
