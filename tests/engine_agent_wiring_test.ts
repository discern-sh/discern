...
Deno.test("Claude refresh replaces renamed and already-duplicated lifecycle hooks", async () => {
  await withTempDir(async (dir) => {
    await scaffoldEngine(dir, { agents: ["claude_code"] });
    const path = join(dir, ".claude/settings.json");
    const settings = JSON.parse(await Deno.readTextFile(path)) as {
      hooks: Record<
        string,
    ...
    settings.userPreserved = true;
    for (const rename of renames) {
      const groups = settings.hooks[rename.event];
      assert(groups !== undefined);
      const current = groups.find((group) =>
        group.hooks.some((hook) => hook.command === rename.canonical)
      );
      assert(current !== undefined);
      const legacy = structuredClone(current);
      for (const hook of legacy.hooks) {
        if (hook.command === rename.canonical) {
          hook.command = rename.retired;
        }
      }
      groups.unshift(legacy);
    }
    settings.hooks.WorktreeCreate?.unshift({
    ...
    assert(
      JSON.parse(refreshed.stdout).data.hooks_wired.includes(
        ".claude/settings.json",
      ),
    );

    const converged = JSON.parse(await Deno.readTextFile(path)) as {
      hooks: Record<
        string,
        Array<{ hooks: Array<{ type: string; command: string }> }>
      >;
    ...
    assertEquals(converged.userPreserved, true);
    for (const rename of renames) {
      const commands = (converged.hooks[rename.event] ?? []).flatMap((group) =>
        group.hooks.map((hook) => hook.command)
      );
      assertEquals(
        commands.filter((command) => command === rename.canonical).length,
        1,
      );
      assertEquals(commands.includes(rename.retired), false);
    }
    assertEquals(
      converged.hooks.WorktreeCreate?.some((group) =>
        group.hooks.some((hook) => hook.command === "user-hook")
      ),
      true,
    );
