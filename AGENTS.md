...
- **`discern_accept`** is only for an explicit user handoff/land request. After a green `discern done` run on a completed task, report the change to your owner in your own words, end with the one-line receipt (`data.receipt.line` — the full receipt is theirs to pull with `discern status --verbose`, never pasted into a message), and stop; land only once they accept (or gave you a standing pre-authorization). It fast-forwards the trunk (`main`), refreshes it, runs `[repository].ensure` and `smoke`, reports failures without stopping cleanup, then removes the worktree and branch.
