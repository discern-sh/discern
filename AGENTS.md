...
- **Update through `discern_update`.** Call it when behind `main` mid-work; it checks its preconditions — no pre-check or hand-merge. Re-read files named in its overlap report. A proven revision needs no update: `discern_accept` composes the moved trunk itself.
