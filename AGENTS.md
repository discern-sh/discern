...
- **`discern_accept`** is only for an explicit user handoff/land request. After a green `discern done` run on a completed task, relay the receipt to your owner and stop; land only once they accept, passing `--confirmed` (standing pre-authorization counts). It fast-forwards the trunk (`main`), refreshes it, runs `[repository].ensure` and `smoke`, reports failures without stopping cleanup, then removes the worktree and branch.
...
- **Several artifacts and schemas are generated.** The `deno task codegen` command rewrites `project/map/70-reference/config-reference.md`, `project/map/70-reference/cli-reference.md`, `schema/*.json`, and `types/*.d.ts` automatically. The codegen command is wired in to discern's own `[capabilities.build]` step (the `discern.toml` template stays hand-authored — ADR 0005/0026).
