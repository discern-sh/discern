...
4. **`discern setup done` → unchanged proof.** `refresh → doctor → finish`, then
   ...
   its specific reactivation step (new session; for some, a one-time trust).
   Each step is DERIVED from that agent's required wiring (its live `mcp`
   server, its `hooks`, its project rules, and its `trust` gate) — not a
   hand-listed table — so a reuse-canonical agent that wired nothing is never
   told to restart, and a parity test ties the derivation to the `PROVIDERS`
   registry (ADR 0051): a new vendor's setup-completion handoff follows from its
