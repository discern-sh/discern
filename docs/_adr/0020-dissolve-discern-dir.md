...
2. **It forced the binary's artifacts to cohabit with your files.**
   `.discern/skills/` held **bundled** skills (the binary's, gitignored,
   ...
   `.discern/skills/` would lose them by following the upgrade guide literally.)
3. **There was no provider-agnostic, config-driven way to point discern at your
   _own_ conventions.**
...
3. **An output discern writes** (never yours): the compiled agent files
   (`AGENTS.md` tracked; `CLAUDE.md`/`GEMINI.md` gitignored) and the
   materialized `.claude/skills/`.

Concretely:

- **Root discovery keys on `discern.toml`.** The pre-6 `.discern/config.toml` is
...
- **`.claude/` is the provider's, not discern's.** discern writes hooks and
  materializes skills there because that is where Claude Code looks; it is a
  write-_target_, never part of "where discern keeps its stuff."
...
- **An empty install is one file: `discern.toml`** (plus the generated
...
- **The part-tracked/part-ignored skills bug is gone by construction** (R1): the
  5→6 migration splits `.discern/skills/` by name — authored dirs move to
...
- This **supersedes [ADR 0016]**: the `.discern/` namespace it introduced is
...
- **Keep `.discern/` but un-hide the authored parts.** Rejected: the real
...
- **Keep `AGENTS.md` gitignored like the other mirrors.** Rejected: a tracked
  ...
  standard.
- **A blanket prune of `.discern/skills/` on migration.** Rejected: it would
