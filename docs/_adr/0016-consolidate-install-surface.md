...
`icculus init` into a fresh project scaffolded **66 files across eight top-level
entries**: `bin/`, `docs/`, `TODO.md`, `icculus.toml`, `.ai/`, `.claude/`,
`.icculus/`, and `.gitignore`. The raw count is not the problem — `.git/` is
...
- **The config carried the kit's brand into the project root.** `icculus.toml`
...
Consolidate everything the kit owns under the single `.icculus/` namespace, and
...
1. **One namespace for machinery.** The dispatcher moves to a **root-level
   ...
   **`.icculus/config.toml`**; guidance to **`.icculus/guidelines/`**; skills to
   **`.icculus/skills/`**. The `.ai/` directory is gone. The "is this an icculus
   project?" root marker is now `.icculus/config.toml` (with a legacy fallback
   to `icculus.toml`, see point 4). After `init`, a plain `ls` shows nothing new
   but `agent` and the `.icculus/` namespace.
...
2. **Lazy docs and TODO.** `init` scaffolds **no `docs/` tree and no
   ...
   them, under `.icculus/skills/<skill>/skel/`, and are created on demand:
...
4. **Existing installs migrate, they are not re-initialised.** This is the first
   ...
   new **schema 2 → 3** migration renames `bin/agent` → `agent`, `icculus.toml`
   → `.icculus/config.toml`, `.ai/guidelines` → `.icculus/guidelines`,
   `.ai/skills` → `.icculus/skills`, repoints the `.claude` worktree hooks at
...
- The empty-project blast radius drops from **66 files across 8 entries** to
  roughly **48 files across 4** (`agent`, `.icculus/`, and the two unavoidable
...
- **The legacy `icculus.toml` path is now a recognised input** to `upgrade`/
...
- This amends — does not supersede —
  ...
  now lists `agent` and `.icculus/skills/` rather than `bin/agent` and
...
- **A root `agent` symlink into `.icculus/`.** Rejected: a committed symlink
  ...
  the real script.
- **Keep `icculus.toml` at the project root.** A root config file is a strong
