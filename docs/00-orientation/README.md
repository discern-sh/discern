...
> These four docs start as skeletons. The `/bootstrap` skill fills them from the project brief at `.icculus/brief.md`. Look for the `<!-- /bootstrap fills this -->` markers.
...
- **design-principles.md** is the constitution. When a subsystem doc explains *why* a thing is done a certain way, it usually traces back to a principle here — and any deliberate exception to a principle is recorded as an ADR in [`../_adr/`](../_adr/).
