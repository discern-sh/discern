...
> test stack. Look for the `<!-- /bootstrap fills this -->` marker.
...
_(How the `test` slot invokes the suite, how to run a single test or a filtered
subset while iterating, and — importantly — whether the suite runs in parallel.
If it does, the [finish-gate gotchas](finish-gate-gotchas.md) "passes alone but
