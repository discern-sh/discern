...
| **co-managed seed** | `discern.toml`: project-owned values and record tables, plus discern's fixed documented scaffold.                                                                                           | Versioned migrations run, then missing fixed sections/keys from the current template are restored. Existing values and named record tables are left alone (ADR 0092). | Edit project values and named tables in place.                   |
...
| [`.gitignore`](../../templates/.gitignore.fragment)                   | yours (merged) | Idempotently ignores the compiled agent files `/AGENTS.md`, `/CLAUDE.md`, `/GEMINI.md` (all build artifacts — ADR 0034) and `/.claude/*` except the shared `.claude/settings.json`. Machine-local `.claude/settings.local.json` stays ignored.                                            |
