...
| [`.gitignore`](../../templates/.gitignore.fragment)                   | co-managed       | The project owns its ignore rules outside `# --- discern ---` / `# --- /discern ---`. Inside that block, discern ignores the compiled agent files `/AGENTS.md`, `/CLAUDE.md`, `/GEMINI.md` (all build artifacts — ADR 0034) and the generated skills dirs; `upgrade` reconciles it to the current fragment (ADR 0093), while `status` warns and `finish` blocks if those discern-owned ignored paths are tracked. |
