...
> stack. Look for the `<!-- /bootstrap fills this -->` marker.

This is the path from a fresh clone to a running project and a first green gate.
The harness commands are the same on every stack; the stack-specific steps
...
environment/config setup, how to start the app, and what a first `agent finish`
should produce. Keep these aligned with the `[slots]` and `[worktree]` adapters
in `icculus.toml`.)_
