...
isolated `discern-setup` branch so your `main` is never touched mid-setup. It
lays down the scaffold — `discern.toml`, the `discern/` namespace, the docs
skeleton — records a short brief of what you're building, and prints a **setup
brief** the agent then works through: filling in your guidance, sketching the
first docs, wiring the gate commands for your stack.
