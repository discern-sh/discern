...
and guidance. [`bin/agent`](../../templates/bin/agent) is the **dispatcher**: it
finds the project root (the nearest ancestor with an `icculus.toml`), routes
...
| `config-access.md`     | Reading `icculus.toml` from shell via `config.sh` and `toml.awk`.                     |
