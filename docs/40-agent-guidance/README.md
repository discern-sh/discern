...
source** — `.ai/guidelines/<slug>.md` — and
[`agent guidelines`](../../templates/.icculus/engine/guidelines) compiles it
into each **Compiled agent file** (`CLAUDE.md`, `AGENTS.md`, …), selected by
`[project].agents` in `icculus.toml` (`claude_code` → `CLAUDE.md`, `codex` →
`AGENTS.md`). The same recipe refreshes the `.claude/skills/` symlinks that make
the bundled **Skills** discoverable.
...
always reproduced by re-running the recipe. Driving several agents from one
...
[`.ai/skills/`](../../templates/.ai/skills/) as Managed files.
...
| -------------------------- | -------------------------------------------------------------------------------- |
| `the-guidelines-recipe.md` | How `agent guidelines` compiles the source into per-agent files and banners.     |
