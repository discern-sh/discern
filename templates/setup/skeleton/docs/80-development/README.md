...
- `discern prepare` is the fast inner loop — it applies the fix-stage jobs, regenerates `[generated]` artifacts, then runs the check-stage jobs; build jobs and tests stay with `discern done`.
