...
`init` no longer scaffolds `docs/` or `TODO.md` — the skeletons ship with this skill so the tree is created only when you are about to fill it. Copy them into the project root now:

- `.icculus/skills/bootstrap/skel/docs/` → `docs/`
- `.icculus/skills/bootstrap/skel/TODO.md` → `TODO.md`

(The skill is symlinked at `.claude/skills/bootstrap/`, so its `skel/` is reachable at either path.) Every skeleton uses `{{project_name}}` as a placeholder for the project's name — replace it as you fill each file (you have the name from the brief and `[project].slug`). Leave `skel/` itself untouched; it stays with the skill for the next project.
...
Open **`.icculus/guidelines/<slug>.md`** (the `<slug>` is your project's slug). Flesh out the stub:
...
- **Create the directories with a stub `README.md` each** (a title and a one-line "what this subtree covers"), so the tree is navigable — but don't write the leaves now. Filling a subtree's leaves is the [`document-subsystem`](/.icculus/skills/document-subsystem/SKILL.md) skill's job, run per subsystem when you're ready.
...
- **Point the gate at its gotchas doc.** Step 0.5 created `docs/80-development/finish-gate-gotchas.md`; set `[project].gotchas_doc = "docs/80-development/finish-gate-gotchas.md"` in the config so a non-obvious gate failure points agents at it (`init` leaves `gotchas_doc` empty).
...
4. Fix anything `doctor` flags (it returns the exact remedy), then summarise for the user: the principles you drafted, the subtrees you proposed, the slot fills awaiting their confirmation, and the `TODO.md` items you recorded. Point them at the [`document-subsystem`](/.icculus/skills/document-subsystem/SKILL.md) skill as the next step for filling in each subtree's leaves.
...
- `.icculus/guidelines/<slug>.md` has a real pitch and Conventions section.
...
- `.icculus/config.toml` slot fills are **proposed** for every detected stack (committed only if the user confirms).
