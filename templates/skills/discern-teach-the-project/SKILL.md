...
The judgement this skill holds is *routing*: each kind of lesson has exactly one right home, and a lesson filed in the wrong one is never found again.
...
- **The user corrected you** — your approach was reasonable but wrong *here*; the correction encodes a project rule nobody had written down.
...
- **An authored skill** must be a genuine multi-step playbook — trigger-rich `description` frontmatter (that's what matching runs on), concrete steps with the judgement points called out, and a falsifiable "done when". A single deterministic action is not a skill; make it a recipe.
- **A recipe** is an executable with a `# desc:` line, exiting non-zero on failure, silent about things it didn't do.
...
- A new skill or recipe: confirm it's discoverable — `discern skills list` shows the skill materialized into the agent dirs; `discern --help` lists the recipe.
...
- what was authored meets **that surface's bar** (an earning-its-line guidance rule, a real playbook, an executable recipe, a current doc, a why-carrying ADR);
- the surface is **live** — guidance recompiled, skill/recipe discoverable — and the user was told what the project just learned;
