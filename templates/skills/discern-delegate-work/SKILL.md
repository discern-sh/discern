...
Report what you find plainly: what stands, and what needs another pass. Feed any real defect back as the next task, delegated the same way if that fits. For staged work, this review is also the moment to write the next stage's brief against what landed.
...
- The handoff has the simplest shape that fits: one brief, a fan-out on real seams with disjoint files in flight and a fixed landing order, or staged briefs.
