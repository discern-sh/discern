...
**Include required companion edits in the allocation.** Check ownership exclusions against the live gate before handing off: a stream must be able to flip its own brief's status line and add exact guard enrollments for already-declared paths or capabilities. Name those narrow edits and their landing order when a file is shared. Keep new shared behavior and contract decisions with their assigned owner. A routine enrollment or reference repair that preserves the existing contract needs no second permission exchange.
...
When there's more than one, also offer to save them as Markdown files in the user's project, each filename prefixed with its workstream key (`1a-<slug>.md`, `2a-<slug>.md`), in a spot you suggest from the project's own layout (an existing planning or prompts folder, say). Files keep the key first — the folder scopes them, and the key sorts them into the wave plan; only worktree names lead with the programme slug (§3). Write them only if the user says yes. Open each saved brief with a status line as its first body line — `Status: pending` — and give it one final line in its own definition of done: when its task is complete, flip that line in place to `Status: complete (<date>) — landed as <branch>`, landed as part of that work. The brief is the one authority for its own completion: the file stays where it is and no shared index row is edited, so streams finishing in parallel never write the same file and their landings never contend or re-open questions over an aggregate none of them owns. That line belongs only in a saved brief (a chat-only prompt has no file to flip), and works once the briefs are committed to the trunk each stream branches from.

If you save an index or README beside the briefs, keep it a plain list of links with no per-brief status column: status lives in each brief's own first line, so completing one stream edits only its own file, and the wave still reads at a glance — open the briefs, or scan the status lines (`grep -r "^Status:" <folder>`).
