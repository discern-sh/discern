...
- **Independent streams inside one task → one brief that fans out.** If the task has streams that could run at once and the receiving agent can likely launch its own sub-agents, tell it to do exactly that: "spin up a sub-agent per stream and run them in parallel." Phrase it as a preference with a fallback ("if you can run sub-agents, parallelise these streams; otherwise do them in sequence") because not every agent can. One prompt, one worktree; that receiving agent is the handoff's only dispatch owner.
...
- **Carry staged dependencies.** Name the exact returned branch, the readiness condition, and the composition move from §2. The receiving agent's built-in guidance owns the wait procedure.
...
Then explain the dispatch topology outside the copyable prompts. For a one-brief fan-out, the single receiving agent launches its sub-agents. For a multi-brief handoff, the user launches each prompt as a separate new session by default; if you can launch them directly and the user prefers it, launch each once from this session. Every new session starts its own branch in a fresh worktree, isolated from the other streams. Never ask a receiving agent to launch its sibling briefs. Restate the multi-brief landing order so the user knows which result lands first. Start each lower wave first, capture its exact returned branches, and finalize the already-drafted dependent briefs before dispatching them. When every cross-wave brief carries its wait (§2, §4), the whole set can dispatch in one sitting and run without anyone relaying readiness by hand.
...
- The dispatch owner is explicit: one receiving agent fans out from one brief, or the user or authoring agent launches each brief in a multi-brief set once. Every multi-brief receiver owns only its brief and is told not to launch, dispatch, or supervise its siblings.
