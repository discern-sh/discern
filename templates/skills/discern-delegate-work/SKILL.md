...
- **Wait with the verb.** A brief that depends on a sibling stream has its agent invoke `discern_await` (or `discern await` over the CLI) without a timeout so its configured client uses one longest-safe call and returns early when the condition holds. Once called, do not surface progress updates until it returns. If it returns with `data.met: false`, continue with `data.resume` without surfacing an update. Follow the returned resume hint or `--resume` command. Repeat with no fixed retry limit until the condition holds, the user stops the watch, or the task no longer needs it. Reserve an explicit timeout for a caller with a tighter request budget. An `ok: false` refusal has no continuation. Do not resume it. Follow its recovery hint. Report only when the condition holds, the task no longer needs the watch, or the call returns a refusal or error that needs action. Always respond to new user input. Never use a hand-rolled sleep loop or require a human relay.
