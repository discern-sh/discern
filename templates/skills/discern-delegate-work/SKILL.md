...
A saved brief is the user's file. `discern refresh` rebuilds the generated agent files and never touches it, so when discern's workflow changes, the project's active briefs keep their old sequences until someone revises them. Offer that revision: bring each active brief to the current sequence while keeping every real dependency, chosen order, and review the user asked for.
