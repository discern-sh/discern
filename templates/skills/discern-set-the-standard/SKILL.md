...
When the owner agrees that the breach should become a proposal, first complete every required preview, review, regeneration, edit, `discern_prepare` run, and ordinary commit. Proposal creation is a finalization step, not an intermediate checkpoint. Call **`discern_standards`** with `action: "propose"` and every simultaneously approved breach in one ordered `proposals` array. The transaction measures the same clean final `HEAD` through the shared planner, creates one config-only commit, and binds every proposal to that commit for Proof and acceptance. The terminal-compatible scalar form is `discern standards propose <name> --reason "…"` when exactly one breach is being recorded.

Write each reason as technical justification only, in one visible secret-free paragraph of at most 500 characters; aim below 400. Never put approval, consent, or landing authority in the reason. If required follow-up work later creates a descendant and the proposal set remains unchanged, repeat the complete MCP batch to remeasure and renew it atomically without changing Git history. The scalar terminal command can renew one proposal under the same conditions. A changed value or reason is a different decision: present the new value, delta, and reason to the owner, obtain fresh agreement, follow the refusal's restoration instructions, and create the new proposal only after the tree is final.
