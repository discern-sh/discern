...
A rule that needs judgment rather than measurement ("is this large cut proven safe?") belongs in a checkpoint — the `discern-set-a-checkpoint` skill walks that authoring; when a checkpoint's criterion later becomes mechanically decidable, it graduates here through the outlaw procedure.
