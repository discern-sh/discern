...
- `discern worktree` provisions an isolated checkout for a change (see the worktree note in the project guidelines).
- `discern tidy` is the fast inner loop — it applies the fix-stage capabilities, then the check-stage capabilities, and never builds or tests.
- `discern finish` is the full gate: it runs the fix/build-stage capabilities, then `check` and `test` in parallel, fires any scope `gate`s that fired, and (in a worktree) verifies your branch contains the latest `main`. Run it before declaring any change done; fix what it reports and re-run.
...
| `getting-started.md` | _Skeleton — fill during `/bootstrap`._ Cloning, the worktree step, environment setup, running the app, the first `discern finish`. |
