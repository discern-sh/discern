...
- **`icculus worktree`** sets up an isolated checkout for a change (see the worktree note in the project guidelines).
- **`icculus tidy`** is the fast inner loop — applies the fix-stage capabilities, then the check-stage capabilities; no build, no tests.
- **`icculus finish`** is the full gate — fixers and build, then checks and tests in parallel, then any scope `gate`s that fired, then (in a worktree) the merge check. Run it before declaring a change done.
- **`icculus doctor`** verifies the install is sound (dispatcher executable, hooks present, every configured capability resolvable, git worktree support, required tools on PATH).
