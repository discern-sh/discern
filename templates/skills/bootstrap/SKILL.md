...
1. Run **`discern guidelines`** to compile the built-in harness guidance + `guidance.md` into the per-provider agent files (`AGENTS.md` tracked, `CLAUDE.md` / `GEMINI.md` gitignored) and materialize the skills into `.claude/skills/`.
...
- `discern guidelines` and `discern doctor` have been run and `doctor` is green.
