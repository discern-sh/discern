...
Triggers are deterministic and closed. There is no expression language. The common choices below guide placement; for the complete ordered field model and copyable recipes, run `discern docs checkpoints --raw`. The generated config reference remains the field authority.
