...
> This doc is a skeleton. The `/bootstrap` skill (and the [`document-subsystem`](../../.ai/skills/document-subsystem/SKILL.md) skill, when filling the `80-development` subtree) writes it from the project's actual test stack. Look for the `<!-- /bootstrap fills this -->` marker.

The `test` slot in `icculus.toml` is what `agent finish` runs; this doc explains how to write tests that pass it and how to run them while iterating.

## How tests run

<!-- /bootstrap fills this -->

_(How the `test` slot invokes the suite, how to run a single test or a filtered subset while iterating, and — importantly — whether the suite runs in parallel. If it does, the [finish-gate gotchas](finish-gate-gotchas.md) "passes alone but fails in the full run" trap applies: every test must own its fixtures and assume no ordering. Spell out the parallel-safe pattern this project uses.)_
