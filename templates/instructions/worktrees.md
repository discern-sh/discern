...
**`discern_accept` submits and lands.** It records this effort's submission — the exact proven commit — and lands it on `{{main_branch}}` under verified authority: conversation consent (`--confirmed`), a standing scope grant on `{{main_branch}}`, or the owner's desk-recorded effort grant. A passing gate is evidence, never consent, and neither is a task brief or handoff; follow `done`'s next action. Without authority it refuses read-only and the submission waits; relay the Proof line and stop. Landing removes this worktree, its resources, and its branch once it holds nothing beyond the landed submission. If `{{main_branch}}` moved after your Proof, `accept` re-checks the combined code in its own disposable integration worktree and lands that exact result, waiting its turn behind another landing. A conflict or failed combined check returns the files or check named, nothing landed: run `discern_update`, resolve, commit, `discern_done`, `discern_accept` again. Never adopt an `integration/` worktree — discern's own disposable copy.
