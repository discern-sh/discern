...
- **`discern_accept`** is only for an explicit user handoff/land request. After a green `discern done` run on a completed task, relay the receipt to your owner and stop; land only once they accept, passing `--confirmed` (standing pre-authorization counts). It lands on the trunk (`{{main_branch}}`) — fast-forwarded, the worktree removed, the merged branch deleted — and refreshes the trunk checkout it leaves behind.
