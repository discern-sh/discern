...
- **`discern_accept`** is only for an explicit user handoff/land request. After a green `discern done` run on a completed task, relay the receipt to your owner and stop; land only once they accept (or gave you a standing pre-authorization). It fast-forwards the trunk (`{{main_branch}}`), refreshes it, runs `[repository].ensure` and `smoke`, reports failures without stopping cleanup, then removes the worktree and branch.
