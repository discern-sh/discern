...
- **`discern_accept`** is only for an explicit user handoff/land request, or when a discern result reports machine-verified landing authority. After a green `discern done` run on a completed task, report it — your words, then the one-line receipt — and stop; land only once they accept in conversation or discern verifies a grant recorded in `[acceptance]` or at the desk. `confirmed` attests only to conversation consent; recorded grants are checked directly. Landing fast-forwards `{{main_branch}}` and removes the worktree and branch.
