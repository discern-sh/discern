...
(express, hono, cors, jose, and their closures) on the reasoning that they are
...
1. **JSR components** are the packages behind the graph's `https://jsr.io/…`
   ...
   set the compiler materializes. Packages in the snapshot but outside the
   closure (other workspace roots) are excluded; type-only roots stay in,
...
2. **License texts ship verbatim, from the packages themselves.** npm texts come
   ...
   the label); JSR texts are fetched once from jsr.io — which requires every
   ...
   loudly; `LICENSE_OVERRIDES` is the deliberate, per-version escape hatch
   (empty today), never a second manifest.
...
4. **The Deno runtime is credited.** The binary also embeds the runtime; a fixed
   section credits the Deno authors (MIT) and points to Deno's own published
   license and notices rather than enumerating its Rust-crate closure.
5. **Guards hold the tie** (`tests/third_party_notices_test.ts`): offline
   ...
   the gate); closure-closedness against `deno.lock` as an independent oracle (a
   generator regression that silently drops part of the embedded closure — this
   ADR's originating defect class — fails even though regeneration is
...
- The notices honestly list the full embedded closure (about 114 components
  today, express and hono included). Long attribution files are the norm, not a
  defect.
