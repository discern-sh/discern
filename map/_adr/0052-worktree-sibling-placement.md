...
- **The convention has exactly one home.** Placement is computed in
  ...
  guard ([`tests/agent_agnostic_test.ts`](../../tests/agent_agnostic_test.ts))
  are untouched — the guard now also scans the shared engine-test harness so a
  test can't reintroduce the old `.claude/worktrees` assumption either.
