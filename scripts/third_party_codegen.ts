  ...
  if (fileName === undefined) {
    throw new Error(
      `${identity} ships no LICENSE file (looked in ${dir}) — ` +
        "add a LICENSE_OVERRIDES entry in scripts/third_party_codegen.ts with the license text from the project's repository",
    );
  }
  ...
  if (license === undefined) {
    throw new Error(
      `${identity} declares no license field and its ${fileName} is not a recognized license — ` +
        "add a LICENSE_OVERRIDES entry in scripts/third_party_codegen.ts",
    );
  }
  return {
    name: pkg.name,
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `GET ${url} returned ${response.status} — ` +
          "add a LICENSE_OVERRIDES entry in scripts/third_party_codegen.ts with the license text from the package's repository",
      );
    }
...
function renderBundleModule(
  ...
  return [
    ...
    "// See scripts/third_party_codegen.ts for how and why.",
    "",
