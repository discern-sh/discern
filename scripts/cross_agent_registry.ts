/**
 * The cross-agent behavior reference as a canonical registry: the researched
 * coding agents, the classified behavioral dimensions, and every headline
 * finding live here as one typed set, and the maintainer reference under
 * `project/map/_internal/` compiles from it (ADR 0257).
 *
...
export interface ReferenceAgent {
   ...
   * researches; absent for a researched agent outside discern's integrations. */
...
export const BEHAVIOUR_CATEGORIES = [
  ...
  { id: "mcp", title: "MCP client behavior" },
...
/** One behavioral dimension: a classified section of the reference. */
    ...
    headline: [
      ...
      },
      {
        ...
        cells: {
          ...
          "codex": "Yes",
          "cursor": "Yes (+ `.claude`/`.codex` compatibility)",
...
Each researched agent fixes its workspace or writable root at launch. Claude Code and Copilot can move that root mid-session ({{s:mid-session-reroot}}).

### Self-granting trust from committed config

| Agent              | Can a cloned repo self-trust?                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Claude Code        | Workspace trust gates committed \`.claude/settings.json\` and \`.mcp.json\`; a named \`enabledMcpjsonServers\` entry applies after trust. |
...
| ------------------ | ------------------------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------- |
| Claude Code        | \`.mcp.json\` (project)                       | \`mcpServers\` map (stdio \`command\`/\`args\`)                                           | workspace trust, then named pre-approval via \`enabledMcpjsonServers\` |
...
Each agent reads a committable project MCP file. Claude's named pre-approval suppresses the additional project-server prompt only after workspace trust, while normal MCP tool permissions remain. The other providers retain their own trust and approval boundaries.
...
**discern decision, 2026-08-03.** Keep the portable server contract in its returned definitions and put the gateway and lifecycle at the start of \`instructions\`; Codex's 512-character instructions makes that ordering useful beyond Claude Code's 2KB bound. Do not expose a public \`[mcp]\` setting or emit per-tool Anthropic metadata. For evaluation only, \`DISCERN_EXPERIMENTAL_MCP_PRELOAD=1\` maps the configured providers into the shared entry: \`alwaysLoad: true\` for Claude Code and \`deferTools: "never"\` for Copilot. The default output remains unchanged, and no mapping is attempted for Codex, Cursor, or Gemini. This environment-only seam can be removed or promoted if provider support converges.`,
  ...
  {
    id: "mcp-result-channel-delivery",
    title: "How clients deliver result fields to the model",
    category: "mcp",
    headline: [
      {
        label: "Model-facing MCP result fields",
        cells: {
          "claude-code": "\`structuredContent\` preferred (observed)",
          "codex": "Both; oversize event copy becomes text-only",
          "cursor": "Structure reaches model; hooks omit it",
          "gemini": "\`content\` only",
          "copilot": "Both; literal JSON mirrors deduplicated",
        },
      },
    ],
    body:
...
const TITLE = "Coding-agent behavior reference";
...
>
> **Freshness.** Flags, defaults, and tool surfaces change between releases. Most behaviors are pinned to mid-2026. MCP schema discovery was re-verified on 2026-08-02 against Claude Code 2.1.212, Codex \`rust-v0.146.0\`, Cursor's current documentation, Gemini CLI v0.53.1, and Copilot CLI 1.0.77. MCP result-field delivery was re-verified on 2026-08-14 using the sources named in {{s:mcp-result-channel-delivery}}. Re-verify a cell before relying on it as current.`;
...
const MATRIX_COMMENTARY = `The matrix supports these cross-cutting conclusions.
...
**Committed configuration and trust.** Codex forbids self-trust from repository configuration. Cursor needs \`--trust\` in print mode. Gemini ignores project \`.gemini/settings.json\` (hooks, MCP, and \`.env\`) in safe mode until the folder is user-trusted. Copilot stores trust in a user-level \`trustedFolders\` list. Claude Code also waits for workspace trust before loading committed settings and MCP configuration; its named server pre-approval applies afterward. A committed hook or MCP entry is therefore inert on a fresh clone until its provider trust boundary is satisfied. Instruction files remain readable regardless of trust.
...
export function resolveSectionTokens(markdown: string): string {
  ...
  return markdown.replace(
    ...
    (token, id: string) => {
      ...
      if (number === undefined) {
        throw new Error(`${token} cites no behavior dimension`);
      }
      return `§${number}`;
    },
  );
