...
| Cursor Agent CLI / ACP  | Current CLI build \`2026.07.23-e383d2b\` uses a 60,000 ms bound. Cursor calls the bundled software development kit (SDK) with no request options, so the SDK's default becomes the effective fixed bound.             | No supported per-server timeout setting. The 60 seconds is the unoverridden SDK default.                                                                                          | \`resetTimeoutOnProgress\` remains false, so progress does not renew the timer.        |
