...
| Agent              | Mechanism                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude Code        | \`EnterWorktree\`/\`ExitWorktree\` tools (own worktrees under \`.claude/worktrees/\`), \`--worktree\`, and a \`WorktreeCreate\`/\`WorktreeRemove\` **hook contract** for version-control-system (VCS) agnostic isolation. Its lifecycle hook is harness-drivable.                                                                                                                                                                                                                                                                                                                                                                                                                |
...
Claude Code exposes an explicit, VCS-agnostic \`WorktreeCreate\`/\`WorktreeRemove\` hook contract that a harness can drive. Codex's \`environment.toml\` fires committable \`[setup]\` and \`[cleanup]\` scripts for Codex-managed worktrees. Each agent has a committable \`SessionStart\` hook ({{s:committable-config}}) that can run per-worktree setup. Teardown reliability varies: Claude's \`WorktreeRemove\` and Codex's \`[cleanup]\` are committable remove hooks, with an open Codex bug at \`#19480\`; Gemini's \`SessionEnd\` is advisory; Cursor's CLI \`sessionEnd\` remains unverified; and Copilot's \`sessionEnd\` fires. A harness that manages its own worktrees should drive creation and removal, point the agent at the worktree through its launch-directory flag, and use \`SessionStart\` for setup. It can use the full \`[setup]\` and \`[cleanup]\` pair for Codex-managed worktrees.
...
Each agent gives a harness a committable hook for setup at session or worktree start. Teardown reliability and scope vary. Claude Code provides \`WorktreeRemove\`. Codex provides an \`environment.toml\` \`[cleanup].script\` for Codex-managed worktree archives, with an open reliability bug at \`#19480\`. Copilot's CLI \`sessionEnd\` fires, Gemini's event is advisory, and Cursor's CLI firing remains unverified. A harness that requires teardown should also reclaim out of band, as discern does through a \`prune\` sweep.`,
  },
  {
...
const LEDE =
  `_A maintained reference to the coding-agent differences a stack-neutral harness must handle: hooks, sandboxes, working-directory and session semantics, worktree mechanisms, and configuration surfaces. It gives maintainers one evidence-backed place to answer why agents behave differently._`;
