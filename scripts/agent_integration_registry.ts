...
export function seamNarratives(): readonly SeamNarrative[] {
  ...
  return [
    ...
    },
    {
      ...
      body:
        `Each \`HooksIntegration\` declares its commands, semantic event kinds, vendor group shape, matcher syntax, timeout key and unit, and merge strategy. One renderer supplies setup, refresh, uninstall, and the tracked templates. Every declared hook receives the separate 600-second budget in the vendor's unit. Claude Code declares session start plus create/remove; Codex joins startup/resume as a regex; Gemini renders startup/resume as separate exact matches in milliseconds; Cursor and Copilot declare their group-level forms. The parity guard iterates the registry and rejects any template, command, event, matcher, or timeout drift. Codex also provides \`environment.toml [cleanup]\` for its app-managed worktrees (see {{g:lifecycle-teardown}}).`,
    },
    {
...
- **PATH auto-detect (ADR 0069).** \`binaries\` lists the agent's matching CLI executables; \`setupPresence\` adds editor-only installation evidence. \`detectAgentsOnPath()\` scans \`PATH\`, and \`discern setup begin\` seeds a fresh install's \`[project].agents\` from the detected set or \`DEFAULT_AGENTS\`. Setup persists that result in configuration, and runtime resolution reads the recorded value.
