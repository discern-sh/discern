...
- The user has a working or proposed solution and asks `/engineering-overkill`, `/engineering-overkill --max`, `/engineering-overkill --advanced`, or `/engineering-overkill --current`.
...
When the user invokes `/engineering-overkill` without any flags, **pause and ask which modes to apply before producing output.** Present the three modes as a multi-select picker (or, in environments without structured prompts, a numbered list) so the user can opt in to any combination — or none, for default behavior. Use this exact phrasing for each option:
...
After the user selects (any subset, including nothing), proceed to produce the response with the chosen modes applied. The selection is per-invocation — do not carry it across future `/engineering-overkill` calls in the same conversation; ask again each time.

If the user invokes `/engineering-overkill` with **any** flag already passed (e.g., `/engineering-overkill --max`, `/engineering-overkill --current`, `/engineering-overkill --advanced --current`), skip the picker entirely and run with exactly the flags they passed. The picker exists to surface modes to users who do not know they exist; users who pass flags have already chosen.
...
Flags can be combined freely: `/engineering-overkill --max --advanced --current`.
...
**User**: "I'm deduplicating webhook deliveries using an in-process set keyed by delivery ID. /engineering-overkill"

**Output** (abbreviated):

> **Baseline:** an in-process set keyed by delivery ID, used to deduplicate webhook deliveries on a single node.
>
> #### Shared key/value store with TTL — Complexity: 🔥 2/10
> **What it is:** record each delivery ID in a shared in-memory store (e.g. a key with a set-and-expire) providing a dedup window visible to every worker.
> **How it goes beyond the baseline:** dedup state survives process restarts and is consistent across multiple delivery workers.
> **Skills developed:** shared-cache data-structure idioms, TTL-based memory budgeting, designing for partial-availability of a shared store.
> **Learn more:** primary documentation for whichever shared store you adopt; a canonical write-up on cache-as-coordination patterns.
...
>
> #### Roaring Bitmaps over a compacted log topic — Complexity: 🔥 8/10
> **What it is:** delivery IDs are hashed into dense integer space and tracked in Roaring Bitmaps, with the authoritative state replayed from a log-compacted topic so any consumer can deterministically rebuild the dedup set.
...
> **Skills developed:** compressed bitmap engineering, hash-distribution analysis, log-compaction semantics, replayable state-machine design.
> **Learn more:** [Roaring Bitmaps paper (Chambi et al., 2014)](https://arxiv.org/abs/1402.6407); primary documentation on log compaction for your chosen log/stream system.
