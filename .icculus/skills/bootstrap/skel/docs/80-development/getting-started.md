...
_(The stack-specific steps: prerequisites to install, how to fetch dependencies, environment/config setup, how to start the app, and what a first `agent finish` should produce. Keep these aligned with the `[slots]` and `[worktree]` adapters in `.icculus/config.toml`.)_
