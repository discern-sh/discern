 ...
 * under `deno run` (that shape is for `deno serve`, which local dev uses), so
 * the entrypoint has to start the server itself. This wraps the identical
 * handler in `Deno.serve` and nothing else — no site behaviour lives here.
 *
