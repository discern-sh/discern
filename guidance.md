...
## Decisions
Architecture decisions live in `docs/_adr/` (0001+, several dozen and counting) — browse them with `discern help --adr`. Add one for any notable or hard-to-reverse change. Two foundations are worth knowing up front: the single-binary TS engine ([ADR 0019](docs/_adr/0019-single-binary-ts-engine.md)) and the dissolution of `.discern/` into the root `discern.toml` ([ADR 0020](docs/_adr/0020-dissolve-discern-dir.md)). The series moves fast, so skim the most recent few before a significant change — a current ADR usually explains why something is the way it is.
