 ...
 * a criterion as a matching change completes, so new violations meet a
 ...
 * must target a criterion whose violations are diff-introduced — an accrued
 * criterion (staleness, absence) throws at construction and is skipped at
...
/** The canonical criterion a configured entry serves, or undefined when the
 * entry authors its own prose (an override IS an authored criterion — the
 * canonical mark would claim protection the boundary does not give). */
function servedCanonicalCriterion(
  entryCriterion: string | undefined,
  seed: BuiltInCheckpointSeed | undefined,
): string | undefined {
  if (seed === undefined) {
    return undefined;
  }
  return entryCriterion !== undefined && entryCriterion.trim() !== ""
    ? undefined
    : seed.criterion;
}

/**
 * Canonical criterion id → the configured checkpoints serving it verbatim.
 ...
 */
export function boundaryGuardsByCriterion(
...
export function boundaryLine(guards: readonly BoundaryGuard[]): string {
  ...
  } this criterion as a matching change completes — new violations meet the ` +
...
function routeConverts(route: GraduationRoute): boolean {
  return criterionById(route.criterion)?.violations === "diff-introduced";
}

/**
 * Construct one graduation route, refusing a route the conversion rule
 * forbids: the criterion must exist and its violations must be introduced by
 * diffs. An accrued criterion (staleness, navigability, missing discovery)
...
export function graduationRoute(route: GraduationRoute): GraduationRoute {
  const criterion = criterionById(route.criterion);
  if (criterion === undefined) {
    throw new Error(
      `graduation route '${route.detector}' references no canonical criterion '${route.criterion}'`,
    );
  }
  if (criterion.violations !== "diff-introduced") {
    throw new Error(
      `graduation route '${route.detector}' targets '${route.criterion}', whose ` +
...
const VARIANCE_WHY =
  ...
  "conclusion; it never records the criterion as met. Repeated variances are " +
...
export function checkpointRecommendations(
  ...
  for (const summary of evidence.varied) {
    ...
    recommendations.push({
      ...
      action:
        ...
        `trigger, its criterion wording, or its mode should move — or whether ` +
  ...
  const guarded = boundaryGuardsByCriterion(config, seeds);
  for (const route of routes) {
    ...
    if (guarded.has(route.criterion)) {
    ...
    recommendations.push({
      ...
      subject: route.criterion,
      title: `Recurring findings match criterion '${route.criterion}'`,
      action: `Decide whether to capture this as a checkpoint: pair the ` +
        `'${route.criterion}' criterion with a deterministic trigger under ` +
        ...
        `verdict.`,
      why:
        `Violations of this criterion arrive with a diff, so the conversion ` +
