...
import {
  type StatusMaterialCurrency,
  statusMaterialCurrency,
} from "./material_currency.ts";
...
interface HintContext {
  ...
  liveCount: number;
  materialCurrency: StatusMaterialCurrency;
...
async function buildStatusHints(ctx: HintContext): Promise<FiredHint[]> {
  ...
  hints.push(...ctx.materialCurrency.artifactHints);
  ...
  hints.push(...ctx.materialCurrency.adoptionHints);
...
export async function runStatus(
  ...
  if (root === undefined) {
    if (opts.json) {
      emitResult(noProjectResult("status"));
    } else {
