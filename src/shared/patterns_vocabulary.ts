...
export const CompletionEconomicsSchema = z.strictObject({
  ...
  extractor_executions: z.number().int().nonnegative().nullable().optional(),
  producer_results: z.record(z.string(), z.number().int().nonnegative())
    .optional(),
  producer_result_missing: z.number().int().nonnegative().optional(),
  unknown_command_identity: z.number().int().nonnegative().optional(),
  conflicting_command_identities: z.number().int().nonnegative().optional(),
  unmatched_command_results: z.number().int().nonnegative().optional(),
  producer_work_ms: z.number().nonnegative().nullable().optional(),
  latencies: z.record(
    z.string(),
    z.strictObject({
      observations: z.number().int().nonnegative(),
