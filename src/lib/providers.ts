...
/**
 * Claude Code's guidance file rendered as a pointer to the canonical (tracked)
 * agent file: a do-not-edit banner plus an `@<path>` import line, which Claude
 * Code expands in place when it loads `CLAUDE.md`. So the compiled guidance lives
 * in ONE file (`AGENTS.md`) and the Claude mirror can never drift from it — the
 * same mechanism a user's own `~/.claude/CLAUDE.md` uses to `@`-import a shared
 * source.
 */
export function claudeCodePointer(canonicalPath: string): string {
  return [
    "<!-- GENERATED by `discern refresh` — do NOT edit this file.",
    `     The agent guidance is compiled into ${canonicalPath} (the tracked,`,
    "     single-source agent file). This Claude Code mirror just imports it via",
    "     the `@` include below — so the two can never drift. To change guidance,",
    "     edit your [guidance].sources (or discern's built-ins) and re-run",
    "     `discern refresh`. -->",
    "",
    `@${canonicalPath}`,
    "",
