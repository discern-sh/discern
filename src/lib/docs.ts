...
/** Replace both generated record lists without changing the authored framing. */
export function replaceAdrIndexBlocks(
  document: string,
  blocks: AdrIndexBlocks,
): string {
  const current = replaceAdrIndexBlock(
    document,
    ADR_CURRENT_RECORDS_START,
    ADR_CURRENT_RECORDS_END,
    blocks.current,
  );
  return replaceAdrIndexBlock(
    current,
    ADR_SUPERSEDED_RECORDS_START,
    ADR_SUPERSEDED_RECORDS_END,
    blocks.superseded,
